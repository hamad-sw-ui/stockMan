...
## 9. État de livraison — Track I (i18n FR/EN) ✅ _livrée_
...
| **I3** | ✅ _livrée_ | **Lot « Catalogue & Stock »** intégralement converti (10 pages) : `vendor/StockPage`, Catégories, Unités, Mouvements, Produits (liste + formulaire + détail), Réceptions (avec l'éditeur de séries), Dépôts, Inventaire (campagnes à cycle complet, ajustement rapide, tournant ABC). Constantes de libellés remplacées par des ids stables + `t()` (`pages.inventory.status.*`, `scope.*`, `reasons.*`), dates/monnaies/quantités inchangés via `format.*`. Les chaînes contractuelles CSV (en-têtes FR parsés par l'API, gabarits de zone de texte) restent volontairement en dur, repérées par commentaire. Le test responsive bascule ses assertions sur les **clés i18n** (`data-label={t("…")}`) pour les pages converties. | `pages.inventory.*` : 123 clés FR = source historique à l'octet près, miroir EN complet ; tsc ✔ · **129/129** web · build ✔ |

| **I4** | ✅ _livrée_ | **Lots « Ventes & tiers », « Pilotage » et « console éditeur »** intégralement convertis (16 pages) : Ventes (liste + détail), Clients & crédit, Fournisseurs, Factures & avoirs, Devis & proforma, Approvisionnement ; Notifications, Tableau de bord, Sessions de caisse, Équipe, Journal d'audit, Promotions, Rapports (7 onglets analytiques, exports SYSCOHADA) ; Paramètres (entreprise, caisse, balance à pesée, **sauvegarde/restauration**, alertes, compte) et Abonnement ; les **7 écrans de la console éditeur** (`sa/*` : vue d'ensemble, supervision, licences, plans, configurations, tenants, détail tenant avec impersonation). Statuts de licence factorisés en racine `licenseStatus.*` (repli sur le code brut), quotas « utilisateur(s)/dépôt(s) » mutualisés (`pages.subscription.quotasValue`), corps de confirmations via `<Trans>`. Le document légal d'impression facture (NIU/RCCM, HT/TVA/TTC) reste en français contractuel, commenté « hors champ i18n v1 ». | `pages.sales/customers/suppliers/invoices/quotes/purchaseOrders/notifications/dashboard/cashSessions/vendors/audit/promotions/reports/settings/subscription/sa.*` : ~585 clés FR = source historique à l'octet près, miroir EN complet ; tsc ✔ · **129/129** web · build ✔ |

| **I5** | ✅ _livrée_ | **Caisse vendeur + erreurs API + verrouillage qualité.** Pages vendeur intégralement converties : `PosPage` (catalogue/tuiles/variantes/panier/encaissement/carte de succès, capture IMEI — 6 sous-composants), `PaymentsPage`, `CashSessionPage`, `SyncQueuePage`, `ZReportPage` ; méthodes de paiement via `paymentMethodLabel()`, date de journée via `common.dayOf`. **Pont erreurs API** (`lib/http.ts`) : `translateApiError(code, messageServeur)` → `t("errors.<CODE>")` quand la clé existe (`NETWORK`, `SESSION_EXPIRED`…), **repli sur le message serveur** sinon — l'API reste francophone en v1, aucun client existant ne casse ; statuts HTTP interpolés via `errors.generic` / `errors.download`. **Verrouillage qualité** : test de parité stricte FR=EN conservé vert + nouveau **garde-fou anti-chaînes FR en dur** (`tests/noHardcodedFr.test.ts` : balaie `src/**/*.tsx`, signale les libellés JSX francophones littéraux — attributs, gabarits, nœuds texte mono/multi-lignes — avec liste blanche anti-péremption limitée aux imprimés légaux FR ; contrôle négatif vérifié : une chaîne FR injectée fait échouer la suite). Résidus repérés par le garde-fou puis convertis : page 404 (`pages.notFound.*`), modale d'étiquettes complète (`LabelsPrintModal` + helper `labelTemplateLabel()` avec repli FR), aria-label du SVG code-barres, graphiques SVG (`charts.tsx`), champs Code-barres/Chargement du détail produit, Total/TOTAL des devis. Blocs de reçus FR explicitement marqués « hors champ i18n v1 ». **Glossaire métier FR⇄EN + règle de contribution `t()`** documentés dans `docs/README.md` § « Internationalisation ». | `pages.pos/payments/cashSession/syncQueue/zreport.*`, `labelsPrint.*`, `charts.*`, `errors.*` : **1 902 clés par langue**, FR à l'octet près ; tsc ✔ · **135/135** web (129 + 3 pont erreurs + 3 garde-fou) · build ✔ · API **239/239** ✔ |

**Portes Track I (complète, jalon I1→I5) :** web `tsc` ✔ + **135/135** tests + build ✔ · API `typecheck` ✔ + **239/239** ✔. Livré : I1 (socle), I2 (transverse), I3 (Catalogue & Stock), I4 (Ventes, Achats & Pilotage + console SA + Paramètres/Abonnement), I5 (POS vendeur, pont `ApiError.code` → `t("errors.*")` avec repli serveur, garde-fou anti-chaînes FR en dur + parité, glossaire métier FR⇄EN et règle de contribution). **Track I intégralement livrée — le plan 07 (D1–D3 · R1–R3 · I1–I5) est clos.**
