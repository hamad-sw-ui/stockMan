...
describe('formatMoney', () => {
  it('formate en FCFA fr-FR sans décimales superflues', () => {
    ...
    expect(formatMoney(12500).replace(/[\s\u202f\u00a0]/g, ' ')).toBe('12 500 FCFA');
  ...
  it('arrondit les décimales au besoin', () => {
    expect(formatMoney(99.5).replace(/[\s\u202f\u00a0]/g, ' ')).toBe('99,5 FCFA');
  });
});
...
describe('dates', () => {
  ...
  it('formatDateTime ajoute l’heure', () => {
    expect(formatDateTime('2026-08-02T13:45:00.000Z')).toMatch(/^02\/08\/2026 \d{2}:\d{2}$/);
  });
...
describe('formatRelative', () => {
  it('à l’instant / minutes / heures / jours', () => {
    ...
    expect(formatRelative(new Date('2026-08-02T11:59:50Z'))).toBe("à l'instant");
    expect(formatRelative(new Date('2026-08-02T11:30:00Z'))).toBe('il y a 30 min');
