...
import { clearOutboxForTests, enqueueSale, listOutbox } from '../src/lib/offline/outbox';
import { syncOutbox } from '../src/lib/offline/sync';

const payloadOf = (id: string) => ({ items: [], paymentMethod: 'CASH', clientSaleId: id });
...
describe('syncOutbox', () => {
  it('succès : la vente sort de la file', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(201, { sale: { id: 'srv-1' }, deduplicated: false })));
  ...
  it('dédoublonnage serveur (200 deduplicated) : traité comme succès', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(200, { sale: { id: 'srv-x' }, deduplicated: true })));
    const r = await syncOutbox();
  ...
  it('erreur métier 4xx (ex. STOCK_INSUFFICIENT) : FAILED conservée', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => jsonResponse(409, { error: { code: 'STOCK_INSUFFICIENT', message: 'Stock insuffisant.' } })),
    );
    const r = await syncOutbox();
  ...
  it('coupure réseau : entrée conservée, FIFO arrêtée', async () => {
    await enqueueSale({ clientSaleId: 'net-1', payload: payloadOf('net-1'), createdAt: 100 });
    await enqueueSale({ clientSaleId: 'net-2', payload: payloadOf('net-2'), createdAt: 200 });
  ...
  it('5xx : conservée pour réessai ultérieur', async () => {
    await enqueueSale({ clientSaleId: 'srv-5xx', payload: payloadOf('srv-5xx') });
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(500, { error: { code: 'INTERNAL', message: 'boom' } })));
    const r = await syncOutbox();
    expect(r.synced).toBe(0);
