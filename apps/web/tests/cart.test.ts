...
describe("effectiveUnitPrice", () => {
  ...
  it("+ supplément variante", () => {
    expect(
      effectiveUnitPrice({ product: savon, variant: rouge, quantity: 1 }),
    ).toBe(550);
  });
  it("× facteur de l’unité dérivée (carton)", () => {
    expect(
      effectiveUnitPrice({ product: savon, unit: carton, quantity: 1 }),
    ).toBe(6000);
  });
  it("× (1 − remise)", () => {
    expect(
      effectiveUnitPrice({ product: savon, quantity: 1, discountPct: 10 }),
    ).toBe(450);
  });
  ...
  it("remise bornée à [0,100]", () => {
    expect(
      effectiveUnitPrice({ product: savon, quantity: 1, discountPct: 150 }),
    ).toBe(0);
    expect(
      effectiveUnitPrice({ product: savon, quantity: 1, discountPct: -10 }),
    ).toBe(500);
...
describe("cartTotal / changeDue", () => {
  it("somme des lignes, arrondie", () => {
    const lines = [
      makeLine({ product: savon, quantity: 2 }),
      makeLine({ product: riz, quantity: 1 }),
    ];
