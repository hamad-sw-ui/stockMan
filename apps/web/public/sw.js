...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    event.respondWith(fetch(request).catch(() => new Response(JSON.stringify({ error: { code: 'OFFLINE', message: 'Hors ligne.' } }), { status: 503, headers: { 'Content-Type': 'application/json' } })));
    return;
  }
