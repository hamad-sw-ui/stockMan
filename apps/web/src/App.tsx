...
function Splash() {
  return (
    <div className="center" style={{ minHeight: '100vh', flexDirection: 'column', gap: 14 }}>
      <div className="pos-total" style={{ borderRadius: 20, width: 56, height: 56, display: 'grid', placeItems: 'center', fontSize: '1.6rem' }}>📦</div>
...
function NotFound() {
  return (
    <div className="center" style={{ minHeight: '70vh', flexDirection: 'column', gap: 8 }}>
...
const route = (path: string, element: RouteProps['element']) => <Route key={path} path={path} element={element} />;

export default function App() {
  return (
    <Routes>
      <Route path="/" element={<RedirectHome />} />
      {route('/login', <PublicOnly><LoginPage /></PublicOnly>)}
      {route('/inscription', <PublicOnly><RegisterPage /></PublicOnly>)}
      {route('/mot-de-passe-oublie', <PublicOnly><ForgotPasswordPage /></PublicOnly>)}
      {route('/reinitialiser-mot-de-passe', <PublicOnly><ResetPasswordPage /></PublicOnly>)}
