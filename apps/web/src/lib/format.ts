...
const moneyFmt = new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 2, minimumFractionDigits: 0 });
...
export function stockStatusLabel(status: 'ok' | 'low' | 'out'): string {
  return status === 'ok' ? 'En stock' : status === 'low' ? 'Stock bas' : 'Rupture';
}
...
export function movementTypeLabel(t: string): string {
  const map: Record<string, string> = {
    ...
    SALE: 'Vente', RETURN: 'Retour', DAMAGE: 'Casse', EXPIRED: 'Péremption', VOID: 'Annulation',
  };
  return map[t] ?? t;
}
...
export function notificationTypeLabel(t: string): string {
  const map: Record<string, string> = {
    LOW_STOCK: 'Stock bas', EXPIRY: 'Péremption', DAILY_REPORT: 'Rapport du jour', SYSTEM: 'Système', SYNC_FAILURE: 'Échec de synchro',
  };
  return map[t] ?? t;
}
