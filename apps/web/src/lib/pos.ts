  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        depots = all.filter((d) => d.is_active).map((d) => ({ id: d.id, name: d.name, is_active: d.is_active }));
      } catch {
        depots = [];
      }
      if (user?.depotId) {
        depots = depots.filter((d) => d.id === user.depotId);
        if (depots.length === 0) depots = [{ id: user.depotId, name: 'Mon dépôt', is_active: true }];
      }

      const depotId = requestedDepotId ?? user?.depotId ?? depots[0]?.id ?? null;
      if (!depotId) {
        if (alive) setState({ data: null, loading: false, error: 'network', fromCache: false, depotId: null, depots });
        return;
      }

      // 2) Réseau d'abord, cache en secours
      try {
        const data = await get<PosBootstrap>(`/pos/bootstrap?depotId=${depotId}`);
        saveBootstrap(depotId, data).catch(() => undefined);
        if (alive) setState({ data, loading: false, error: null, fromCache: false, depotId, depots });
      } catch {
        const cached = await loadBootstrap<PosBootstrap>(depotId).catch(() => undefined);
        if (alive) {
          if (cached?.data) {
            setState({ data: { ...cached.data, depotId }, loading: false, error: null, fromCache: true, depotId, depots });
          } else {
            setState({ data: null, loading: false, error: 'network', fromCache: false, depotId, depots });
          }
        }
      }
    })();
