...
export function Button({
  ...
  disabled,
  ...rest
}: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: BtnVariant; size?: 'sm' | 'lg'; block?: boolean; loading?: boolean }) {
  const cls = ['btn', `btn-${variant}`, size === 'sm' ? 'btn-sm' : size === 'lg' ? 'btn-lg' : '', block ? 'btn-block' : '', className].filter(Boolean).join(' ');
  return (
    <button className={cls} disabled={disabled || loading} {...rest}>
      {loading ? <span className="spinner" style={{ width: 15, height: 15, borderWidth: 2 }} /> : null}
      {children}
    </button>
  );
}
...
/* ----------------------------------- Card ---------------------------------- */
export function Card({ children, pad = true, className = '', title, actions }: { children: ReactNode; pad?: boolean; className?: string; title?: ReactNode; actions?: ReactNode }) {
  return (
...
/* ---------------------------------- Champs --------------------------------- */
export function Field({ label, hint, error, required, children }: { label?: ReactNode; hint?: string; error?: string; required?: boolean; children: ReactNode }) {
  return (
    <div className="field">
      {label ? (
        <label>
      ...
      {children}
      {error ? <span style={{ color: 'var(--danger)', fontSize: '0.8rem' }}>{error}</span> : hint ? <span className="hint">{hint}</span> : null}
    </div>
  );
}
...
export function Spinner({ label }: { label?: string }) {
  return (
    <div className="center" role="status">
      ...
      {label ? <span className="muted" style={{ marginLeft: 10 }}>{label}</span> : null}
    </div>
  );
}
...
export function Tabs({ tabs, active, onChange }: { tabs: Array<{ id: string; label: ReactNode }>; active: string; onChange: (id: string) => void }) {
  return (
    <div className="tabs" role="tablist">
      {tabs.map((t) => (
        <button key={t.id} role="tab" aria-selected={t.id === active} className={t.id === active ? 'active' : ''} onClick={() => onChange(t.id)}>
          {t.label}
        </button>
      ))}
    </div>
...
export function Pagination({ page, totalPages, total, onPage }: { page: number; totalPages: number; total: number; onPage: (p: number) => void }) {
  ...
  return (
    <nav className="pagination" aria-label="Pagination">
      ...
      <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => onPage(page - 1)}>← Préc.</Button>
      <span>Page {page} / {totalPages}</span>
      <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => onPage(page + 1)}>Suiv. →</Button>
    </nav>
  );
}

export function Kpi({ label, value, sub, tone }: { label: string; value: ReactNode; sub?: string; tone?: 'ok' | 'warn' | 'danger' }) {
  return (
    <div className="card kpi">
      <span className="kpi-label">{label}</span>
      <span className="kpi-value" style={tone ? { color: `var(--${tone === 'ok' ? 'ok' : tone === 'warn' ? 'warn' : 'danger'})` } : undefined}>{value}</span>
...
export function Modal({ title, onClose, children, wide, footer }: { title: ReactNode; onClose: () => void; children: ReactNode; wide?: boolean; footer?: ReactNode }) {
  ...
  return (
    <div className="modal-backdrop" onClick={(e) => e.target === e.currentTarget && onClose()} role="presentation">
      <div className={`modal ${wide ? 'modal-lg' : ''}`} role="dialog" aria-modal="true" ref={ref}>
        <div className="row-between" style={{ marginBottom: 14 }}>
          <h2>{title}</h2>
          <button className="btn btn-ghost btn-sm" onClick={onClose} aria-label="Fermer">✕</button>
        </div>
        {children}
        {footer ? <div className="row" style={{ justifyContent: 'flex-end', marginTop: 16, gap: 8 }}>{footer}</div> : null}
      </div>
    </div>
  );
}
...
export function ConfirmModal({ title, message, confirmLabel = 'Confirmer', danger = true, onConfirm, onClose, loading }: { title: string; message: ReactNode; confirmLabel?: string; danger?: boolean; onConfirm: () => void; onClose: () => void; loading?: boolean }) {
  return (
...
export function PageHeader({ title, sub, actions }: { title: ReactNode; sub?: ReactNode; actions?: ReactNode }) {
  return (
    <div className="row-between" style={{ marginBottom: 16 }}>
      <div>
        <h1>{title}</h1>
        {sub ? <div className="muted" style={{ fontSize: '0.92rem' }}>{sub}</div> : null}
      </div>
      <div className="row">{actions}</div>
    </div>
  );
...
export function ErrorState({ error, onRetry }: { error: { message?: string; status?: number } | null; onRetry?: () => void }) {
  return (
    <EmptyState emoji="⚠️" title={error?.status === 402 ? 'Fonction verrouillée par la licence' : 'Chargement impossible'} action={onRetry ? <Button variant="outline" onClick={onRetry}>Réessayer</Button> : undefined}>
      {error?.message}
    </EmptyState>
  );
}
