...
export function LineChart({
  ...
  const { path, area, dots, max, gridY } = useMemo(() => {
    ...
    const seg = points.map((p, i) => `${i === 0 ? 'M' : 'L'}${x(i).toFixed(1)},${y(p.value).toFixed(1)}`);
    const p = seg.join(' ');
  ...
  if (points.length === 0) return <p className="muted">Aucune donnée sur la période.</p>;

  const top = hover != null ? dots[hover] : null;
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="chart" role="img" aria-label="Évolution des ventes">
        {gridY.map((y, i) => (
          <g key={i}>
            <line className="grid-line" x1={PAD.l} x2={W - PAD.r} y1={y} y2={y} />
            <text className="axis-label" x={PAD.l - 6} y={y + 3} textAnchor="end">
              {formatValue(max * [0.25, 0.5, 0.75, 1][i]!).replace(/\s?FCFA/, '')}
            </text>
          </g>
        ))}
        ...
        {points.map((p, i) =>
          i % Math.ceil(points.length / 7) === 0 || i === points.length - 1 ? (
            <text key={i} className="axis-label" x={dots[i]!.x} y={H - 6} textAnchor="middle">
              {p.label}
            </text>
          ) : null,
        )}
      ...
      {top != null && hover != null ? (
        <div className="muted" style={{ fontSize: '0.85rem', textAlign: 'center' }}>
          {points[hover]!.label} : <strong>{formatValue(points[hover]!.value)}</strong>
        </div>
      ) : null}
    </div>
  );
...
export function BarChart({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="chart" role="img" aria-label="Graphique en barres">
      {[0.5, 1].map((f) => (
        <g key={f}>
          <line className="grid-line" x1={PAD.l} x2={W - PAD.r} y1={PAD.t + ih - f * ih} y2={PAD.t + ih - f * ih} />
          <text className="axis-label" x={PAD.l - 6} y={PAD.t + ih - f * ih + 3} textAnchor="end">
      ...
      {bars.map((b, i) => {
        ...
        return (
          <g key={i}>
            <rect className="bar" style={color ? { fill: color } : undefined} x={x} y={PAD.t + ih - h} width={bw} height={Math.max(h, 1)}>
              <title>{`${b.label} — ${formatValue(b.value)}`}</title>
            </rect>
            <text className="axis-label" x={x + bw / 2} y={H - 8} textAnchor="middle">
              {b.label.length > 9 ? `${b.label.slice(0, 8)}…` : b.label}
            </text>
            {b.value > 0 ? (
              <text className="axis-label" x={x + bw / 2} y={PAD.t + ih - h - 4} textAnchor="middle" style={{ fontWeight: 700 }}>
...
/* -------------------------------- Anneau ----------------------------------- */
const DONUT_COLORS = ['#059669', '#2563eb', '#d97706', '#dc2626', '#7c3aed', '#0891b2'];
...
export function Donut({
  ...
  return (
    <div className="row" style={{ alignItems: 'center', gap: 20 }}>
      <svg viewBox="0 0 124 124" className="chart" style={{ maxWidth: 150 }} role="img" aria-label="Répartition">
      ...
      <div className="legend" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 6 }}>
        {segments.map((s, i) => (
          <span key={i} className="key">
            <span className="swatch" style={{ background: s.color ?? DONUT_COLORS[i % DONUT_COLORS.length] }} />
