...
export function useOutboxCount(pollMs = 12_000): { queued: number; refresh: () => Promise<void> } {
...
function OfflinePill() {
  ...
  useEffect(() => {
    if (online && queued > 0) {
      ...
      void syncOutbox()
        .then((r) => {
          if (r.synced > 0) toast.success(`${r.synced} vente(s) hors-ligne synchronisée(s).`);
        })
        .finally(() => {
          setSyncing(false);
          void refresh();
...
function NotificationBell() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await get<Paged<NotificationRow> & { unread: number }>('/notifications?size=6');
      setItems(r.data);
  ...
  return (
    <div className="menu" ref={ref}>
      <button className="icon-btn" onClick={() => setOpen((o) => !o)} aria-label={`Notifications (${unread} non lues)`}>
        🔔
        {unread > 0 ? <span className="dot-badge">{unread > 99 ? '99+' : unread}</span> : null}
      </button>
      {open ? (
              ...
              <button
                ...
                onClick={async () => {
                  if (n.status === 'SENT' && n.channel === 'IN_APP') {
                    await patch(`/notifications/${n.id}/read`).catch(() => undefined);
                    void load();
                  }
                }}
              >
                <span style={{ fontWeight: n.status === 'SENT' && n.channel === 'IN_APP' ? 700 : 500 }}>{n.message}</span>
...
function adminNav(outbox: number): NavGroup[] {
  return [
    ...
    {
      title: 'Vendre',
      items: [{ to: '/admin/caisse', label: 'Caisse (POS)', ico: '🧾', badge: outbox }],
    },
    {
      title: 'Catalogue',
      items: [
  ...
  return [
    {
      items: [
        ...
        { to: '/caisse/file', label: 'Synchro hors-ligne', ico: '🔄', badge: outbox },
      ],
    },
  ];
}
...
function LicenseBanner() {
  ...
  const days = Math.ceil((new Date(lic.end_date).getTime() - Date.now()) / 86_400_000);
  if (lic.status !== 'EXPIRED' && days > 7) return null;
  const expired = lic.status === 'EXPIRED' || days < 0;
  return (
    <div className="license-banner" role="alert" style={expired ? { background: 'var(--danger-soft)', borderColor: 'var(--danger)', color: 'var(--danger)' } : undefined}>
      <span>{expired ? '⛔' : '⏳'}</span>
      <div>
        {expired ? (
          <>
            <strong>Licence expirée le {formatDate(lic.end_date)}.</strong> Les nouvelles opérations (ventes, stocks) sont bloquées
            tant que l&apos;abonnement n&apos;est pas renouvelé.
          </>
        ) : (
          <>
            <strong>
              {lic.status === 'TRIAL' ? 'Essai' : 'Abonnement'} : {days} jour(s) restant(s)
            </strong>{' '}
...
/* ------------------------------- Coquille ---------------------------------- */
export default function Shell({ variant }: { variant: 'admin' | 'vendor' | 'sa' }) {
  ...
  const toast = useToast();
  const impersonating = sessionStorage.getItem('stockman.impersonating') === '1';

  const groups = useMemo(
    () => (variant === 'sa' ? saNav() : variant === 'vendor' ? vendorNav(queued) : adminNav(queued)),
    [variant, queued],
  );
  const roleLabel = user?.role === 'SUPER_ADMIN' ? 'Super Admin' : user?.role === 'ADMIN' ? 'Gérant' : 'Vendeur';
    ...
    <div className={`app-shell ${navOpen ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar" aria-label="Navigation principale">
        <div className="side-brand">
          <span className="logo-dot">{user?.tenant.logo ? <img src={user.tenant.logo} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 10 }} /> : '📦'}</span>
          <div>
            StockMan
            <small>{variant === 'sa' ? 'Console éditeur' : user?.tenant.name}</small>
          </div>
        </div>
        ...
        <div className="side-user">
          ...
          <button className="icon-btn" style={{ background: 'transparent', borderColor: 'rgba(255,255,255,0.15)', color: '#cbd5e1' }} title="Se déconnecter" aria-label="Se déconnecter"
            onClick={() => {
      ...
      <div className="main-col">
        {impersonating ? (
          <div className="impersonation-banner" role="alert">
            🛠️ Session support éditeur active{user ? ` — vous agissez en tant que ${user.name} (actions journalisées)` : ''}.
            <button className="btn btn-sm" style={{ background: 'rgba(255,255,255,0.14)', color: '#fff' }} onClick={() => void quitImpersonation()}>
              Quitter
            </button>
          </div>
        ) : null}
        <header className="topbar">
          <button className="icon-btn burger" onClick={() => setNavOpen(true)} aria-label="Ouvrir le menu">
            ☰
          </button>
          <OfflinePill />
...
export function PageError({ error, retry }: { error: ApiError | null; retry?: () => void }) {
  if (!error) return null;
  return (
    <div className="empty" role="alert">
      <span className="emoji">⚠️</span>
      <h3>{error.status === 402 ? 'Fonction verrouillée par la licence' : 'Chargement impossible'}</h3>
      <p>{error.message}</p>
      {retry ? (
