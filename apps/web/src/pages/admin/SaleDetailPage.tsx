...
export default function SaleDetailPage() {
  ...
  const doReturn = async () => {
    ...
    try {
      ...
      show(
        "Retour enregistré : les quantités sont revenues en stock.",
        "success",
      );
      setReturnMode(false);
      setReturnQty({});
  ...
  if (q.loading)
    return (
      <div className="wrap">
        <Spinner label="Chargement de la vente…" />
      </div>
    );
  if (q.error || !q.data)
    return (
  ...
  return (
    <div className="wrap" style={{ maxWidth: 960 }}>
      <PageHeader
        title={
          <>
            Vente du {formatDateTime(s.created_at)}{" "}
            {voided ? (
        ...
        <Kpi
          ...
          sub={
            s.amount_received != null && s.payment_method === "CASH"
              ? `monnaie ${formatMoney(Math.max(0, s.amount_received - s.total_amount))}`
              : undefined
          }
        />
      ...
      {!sameDay && !voided ? (
        <p className="banner banner-warn" style={{ borderRadius: 10 }}>
          ℹ️ L’annulation complète n’est possible que le jour même (anti-fraude)
          ; utilisez un <strong>retour partiel</strong> pour corriger après
          coup.
        </p>
      ) : null}
      ...
      {confirmVoid ? (
        ...
        >
          <p>
            La vente passera au statut <strong>annulée</strong> et{" "}
            <strong>le stock sera restitué</strong> au dépôt. L’opération est
            journalisée.
          </p>
      ...
      {returnMode ? (
        ...
        >
          <p className="muted" style={{ marginTop: 0 }}>
            Saisissez les quantités <strong>en unité de base</strong> à remettre
            en stock :
          </p>
          <div className="table-wrap">
            <table>
          ...
          <Field label="Motif">
            <Input
              ...
              placeholder="Produit défectueux, erreur client…"
            />
          </Field>
        </Modal>
      ) : null}
