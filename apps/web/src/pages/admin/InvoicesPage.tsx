 ...
 *  Hors champ i18n v1 : document fiscal légal FR (NIU, RCCM, HT/TVA/TTC). */
  ...
  return (
    <div className="wrap">
      <PageHeader
        ...
        sub={
          q.data
            ? t("pages.invoices.subCount", {
                count: q.data.total,
                ht: formatMoney(totals.ht),
                vat: formatMoney(totals.vat),
              })
            : t("pages.invoices.subDefault")
        }
      />
          ...
          <Field label={t("fields.type")}>
            ...
            >
              ...
              <option value="INVOICE">
                {t("pages.invoices.kindInvoices")}
              </option>
              <option value="CREDIT_NOTE">
                {t("pages.invoices.kindCreditNotes")}
              </option>
            </Select>
          </Field>
        </div>
      ...
      {q.loading && !q.data ? (
        <Spinner label={t("pages.invoices.loading")} />
      ) : rows.length === 0 ? (
        <EmptyState emoji="🧾" title={t("pages.invoices.empty")}>
          {t("pages.invoices.emptyBody")}
        </EmptyState>
      ) : (
        <Card pad={false}>
          <div className="table-wrap">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>{r.depotName}</td>
                    <td>
                      {r.customerName ?? t("pages.invoices.cashCustomer")}
                    </td>
                    <td>
                      {r.kind === "INVOICE" ? (
                        <Badge tone="ok">
                          {t("pages.invoices.badgeInvoice")}
                        </Badge>
                      ) : (
                        <Badge tone="warn">
                          {t("pages.invoices.badgeCreditNote")}
                        </Badge>
                      )}
                      {r.parentNumber ? (
                        <div className="muted" style={{ fontSize: "0.78rem" }}>
                          {t("pages.invoices.refPrefix", {
