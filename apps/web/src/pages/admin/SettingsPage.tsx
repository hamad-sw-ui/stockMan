...
import { Button, Card, Field, Input, PageHeader, Select, Spinner, Tabs } from '../../components/ui';
...
function CompanyTab() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', primaryColor: '#059669', currency: 'FCFA', timezone: 'Africa/Douala' });
    ...
    <Card title="Profil de l’entreprise">
      <div className="grid">
        <div className="row" style={{ alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div className="avatar" style={{ width: 84, height: 84, fontSize: '2.4rem' }}>
          ...
          <div>
            ...
            <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()}>Changer le logo</Button>{' '}
            {logo ? (
              <Button variant="ghost" size="sm" onClick={() => setLogo(null)}>Retirer</Button>
            ) : null}
            <p className="muted" style={{ fontSize: '0.82rem' }}>PNG/JPG, redimensionné à 192 px automatiquement.</p>
          </div>
        </div>
        ...
        <div className="row" style={{ flexWrap: 'wrap' }}>
          <Field label="Couleur principale (white-label)">
            <input type="color" value={form.primaryColor} onChange={(e) => setForm({ ...form, primaryColor: e.target.value })} style={{ width: 64, height: 42, padding: 2, border: '1px solid var(--border)', borderRadius: 10 }} aria-label="Couleur principale" />
      ...
      <div className="row" style={{ marginTop: 10 }}>
        <Button loading={saving} onClick={save} disabled={!form.name.trim()}>Enregistrer</Button>
      </div>
    </Card>
  );
}
...
function AlertsTab() {
  const { show } = useToast();
  const q = useQuery<NotificationSettings>('notif:settings', '/notifications/settings');
  const [f, setF] = useState({ alertPhone: '', alertWhatsapp: '', low: true, expiry: true, daily: true, dailyTime: '20:00' });
  const [saving, setSaving] = useState(false);
  const [test, setTest] = useState<{ channel: 'SMS' | 'WHATSAPP'; phone: string }>({ channel: 'SMS', phone: '' });
  ...
  const sendTest = async () => {
    setTesting(true);
    try {
      await post('/notifications/test', { channel: test.channel, phone: test.phone });
      show('Message de test envoyé (vérifiez le journal des notifications).', 'success');
    } catch (e) {
  ...
  return (
    <>
      <Card title="Destinataires des alertes">
        <div className="row" style={{ flexWrap: 'wrap' }}>
          <Field label="Téléphone SMS (gérant)" hint="Format international recommandé : +2376XXXXXXXX">
            <Input value={f.alertPhone} onChange={(e) => setF({ ...f, alertPhone: e.target.value })} placeholder="+237 6XX XXX XXX" />
          </Field>
          <Field label="Numéro WhatsApp">
            <Input value={f.alertWhatsapp} onChange={(e) => setF({ ...f, alertWhatsapp: e.target.value })} placeholder="+237 6XX XXX XXX" />
          </Field>
        </div>
        <div className="grid">
          <label className="row" style={{ gap: 8 }}>
            <input type="checkbox" checked={f.low} onChange={(e) => setF({ ...f, low: e.target.checked })} /> Alerte stock bas (quotidienne)
          </label>
          <label className="row" style={{ gap: 8 }}>
            <input type="checkbox" checked={f.expiry} onChange={(e) => setF({ ...f, expiry: e.target.checked })} /> Alerte péremptions proches
          </label>
          <label className="row" style={{ gap: 8 }}>
            <input type="checkbox" checked={f.daily} onChange={(e) => setF({ ...f, daily: e.target.checked })} /> Rapport de caisse quotidien à
            <input type="time" value={f.dailyTime} onChange={(e) => setF({ ...f, dailyTime: e.target.value })} style={{ width: 110 }} aria-label="Heure du rapport" />
          </label>
        </div>
        <div className="row" style={{ marginTop: 10 }}>
      ...
      <Card title="Tester la configuration">
        <div className="row" style={{ flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <Field label="Canal">
            <Select value={test.channel} onChange={(e) => setTest({ ...test, channel: e.target.value as 'SMS' | 'WHATSAPP' })}>
              <option value="SMS">SMS</option>
              <option value="WHATSAPP">WhatsApp</option>
            </Select>
          </Field>
          <Field label="Numéro de test">
            <Input value={test.phone} onChange={(e) => setTest({ ...test, phone: e.target.value })} placeholder="+237 6XX XXX XXX" />
          </Field>
          <Button variant="outline" loading={testing} onClick={sendTest} disabled={test.phone.replace(/\D/g, '').length < 8}>
            Envoyer un test
          </Button>
        </div>
        <p className="muted" style={{ fontSize: '0.85rem', marginTop: 8 }}>
          En mode « fournisseur simulé » (par défaut en développement), l’envoi est journalisé mais aucun SMS réel ne part. Configurez Africa’s Talking / WhatsApp côté serveur pour la production.
        </p>
      </Card>
    </>
  );
  ...
  return (
    <Card title={`Mon compte — ${user?.name ?? ''}`}>
      <p className="muted" style={{ marginTop: 0 }}>
        {user?.email} · rôle {user?.role === 'ADMIN' ? 'Administrateur' : 'Vendeur'}
        {user?.license ? ` · licence ${user.license.status} (fin ${new Date(user.license.end_date).toLocaleDateString('fr-FR')})` : ''}
      </p>
      <Field label="Mot de passe actuel" required>
        <Input type="password" value={pw.current} onChange={(e) => setPw({ ...pw, current: e.target.value })} autoComplete="current-password" />
      </Field>
      <div className="row" style={{ flexWrap: 'wrap' }}>
        <Field label="Nouveau mot de passe" required hint="8 caractères minimum, une lettre et un chiffre.">
      ...
      <Button loading={saving} onClick={save} disabled={!pw.current || pw.next.length < 8 || pw.next !== pw.confirm}>
...
export default function SettingsPage() {
  ...
  return (
    <div className="wrap">
      <PageHeader title="Paramètres" sub="Personnalisation de l’entreprise, alertes et sécurité de votre compte" />
      <Tabs
        active={tab}
        onChange={setTab}
        tabs={[
      ...
      />
      {tab === 'entreprise' ? <CompanyTab /> : tab === 'alertes' ? <AlertsTab /> : <AccountTab />}
    </div>
  );
}
