...
export default function ForgotPasswordPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await post<{ message: string; devToken?: string }>('/auth/forgot-password', { email: email.trim() });
      setDone(r.message);
          ...
          {done ? (
            <div>
              ...
              {devToken ? (
                <p style={{ fontSize: '0.88rem' }}>
                  ...
                  <Link to={`/reinitialiser-mot-de-passe?token=${encodeURIComponent(devToken)}`}>
            ...
            <form onSubmit={(e) => void submit(e)}>
              <Field label="Email du compte" required>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="vous@entreprise.cm" autoFocus />
              </Field>
              {error ? (
