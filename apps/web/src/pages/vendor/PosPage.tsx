...
const METHODS: Array<{ id: PaymentMethod; icon: string }> = [
  { id: "CASH", icon: "💵" },
  { id: "MTN_MOMO", icon: "🟡" },
...
export default function PosPage() {
  ...
  const addWeighed = (code: string): boolean => {
    ...
    show(
      t("pages.pos.weighedAdded", {
        name: p.name,
        label: `${w.label}${w.embeddedPrice ? t("pages.pos.embeddedSuffix") : ""}`,
      }),
      "success",
    );
    return true;
  ...
  const addByBarcodeOnline = async (code: string): Promise<boolean> => {
    ...
    try {
      ...
      if (!p) {
        show(t("pages.pos.resolvedMissingLocal"), "error");
  ...
  const onCameraDetect = (code: string) => {
    ...
    if (addByBarcode(code)) {
      show(t("pages.pos.addedToast"), "success");
      return;
    }
    void addByBarcodeOnline(code).then((ok) => {
      if (ok) show(t("pages.pos.addedToast"), "success");
      else show(t("pages.pos.unknownCodeToast", { code }), "error");
    });
  };
  ...
  const finishSale = async (
    ...
    const offlineFallback = async () => {
      await enqueueSale({
        clientSaleId,
        payload,
        label: t("pages.pos.ticketLabel", { date: formatDateTime(at) }),
        total,
      });
      setSold({
        saleId: null,
      ...
      setCart([]);
      setPayOpen(false);
      show(t("pages.pos.offlineQueuedToast"), "info");
    };
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 0) {
        ...
        show(
          e instanceof Error ? e.message : t("pages.pos.saleRefused"),
          "error",
        );
      }
    }
  ...
  if (boot.loading) {
    return (
      <div className="center" style={{ minHeight: "60vh" }}>
        <Spinner label={t("pages.pos.loading")} />
      </div>
    );
  }
    ...
    return (
      <div className="wrap">
        <div className="empty">
          ...
          <h3>{t("pages.pos.catalogDownTitle")}</h3>
          <p>{t("pages.pos.catalogDownBody")}</p>
    ...
    <div className="pos-wrap">
      ...
      <section className="pos-catalog">
        <div className="pos-tools">
          <div className="input-icon" style={{ flex: 1, minWidth: 160 }}>
            ...
            <input
              ...
              autoFocus
              aria-label={t("pages.pos.searchAria")}
            />
          </div>
          {canCameraScan ? (
            <button
              ...
              aria-label={t("pages.pos.cameraScanAria")}
              title={t("pages.pos.cameraScanTitle")}
            >
              📷
            </button>
          ) : null}
          {boot.depots.length > 1 ? (
            <select
              ...
              aria-label={t("pages.pos.depotAria")}
            >
              {boot.depots.map((d) => (
            ...
            <Badge>
              {boot.depots.find((d) => d.id === boot.depotId)?.name ??
                t("fields.depot")}
            </Badge>
          )}
        </div>
        ...
        <div className="pos-grid" style={{ marginTop: 10 }}>
          ...
          {filtered.length === 0 ? (
            <p className="muted">{t("pages.pos.noMatch")}</p>
          ) : null}
        </div>
        {boot.fromCache ? (
          <p className="muted" style={{ fontSize: "0.8rem", marginTop: 8 }}>
            {t("pages.pos.cacheNote")}
          </p>
        ) : null}
      </section>
      ...
      <aside className={`pos-cart pos-bar${cartOpen ? " open" : ""}`}>
        ...
        >
          <span aria-hidden>🧾</span> {t("pages.pos.cartTitle")}
          <span className="pos-cart-count">
            {t("pages.pos.cartCount", {
        ...
        <div className="pos-bar-body">
          <div className="row-between" style={{ marginBottom: 8 }}>
            <h2 style={{ margin: 0 }}>🧾 {t("pages.pos.cartTitle")}</h2>
            {cart.length > 0 ? (
              <Button variant="ghost" size="sm" onClick={() => setCart([])}>
                {t("pages.pos.clearCart")}
              </Button>
            ) : null}
          </div>
          ...
          {cart.length === 0 ? (
            <div className="empty empty-block" style={{ padding: "24px 8px" }}>
              ...
              <h3>{t("pages.pos.emptyCartTitle")}</h3>
              <p>{t("pages.pos.emptyCartBody")}</p>
            </div>
          ) : (
            <>
                  ...
                  <div key={l.key} className="cart-line">
                      ...
                      <div className="row" style={{ gap: 6, marginTop: 4 }}>
                        ...
                        !l.product.requiresSerial ? (
                          <select
                            ...
                            aria-label={t("pages.pos.unitAria")}
                          >
                            {(b?.units ?? []).map((u) => {
                        ...
                        >
                          ...
                          <input
                            ...
                            aria-label={t("pages.pos.discountAria")}
                          />
                        </label>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      {l.product.requiresSerial ? (
                        <>
                          <div className="muted" style={{ fontSize: "0.8rem" }}>
                            {t("pages.pos.cartCount", { count: l.quantity })}
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => removeLine(l.key)}
                            aria-label={t("pages.quotes.removeLineTitle")}
                          >
                            🗑️
                          </Button>
                        </>
                        ...
                        <div className="qty-stepper">
                          <button
                            ...
                            aria-label={t("pages.pos.decreaseAria")}
                          >
                            −
                          </button>
                          ...
                          <button
                            ...
                            aria-label={t("pages.pos.increaseAria")}
                          >
                            +
                          </button>
                        </div>
              ...
              <div className="cart-pay">
                ...
                <div className="pay-grid pos-bar-actions">
                  {METHODS.map((m) => (
                    ...
                    >
                      {m.icon} {paymentMethodLabel(m.id)}
                    </Button>
                  ))}
                </div>
                {!online ? (
      ...
      {scanOpen ? (
        <Modal
          title={t("pages.pos.scanModalTitle")}
...
function ProductTile({
  ...
  return (
    <button
      ...
      disabled={stock <= 0}
      title={stock <= 0 ? t("pages.pos.tileOutTitle") : name}
    >
      ...
      >
        {stock <= 0
          ? t("pages.pos.tileOutBadge")
          : t("pages.pos.tileStock", { qty: formatQty(stock) })}
      </span>
    </button>
  );
}
...
function VariantPicker({
  ...
  return (
    <Modal
      title={t("pages.pos.variantTitle", { name: productName })}
      onClose={onClose}
    >
          ...
          return (
            ...
            >
              ...
              <span className="muted">
                ·{" "}
                {stock <= 0
                  ? t("pages.pos.variantOut")
                  : t("pages.pos.variantStock", { qty: formatQty(stock) })}
              </span>
            </Button>
          );
        })}
  ...
  return (
    <Modal
      ...
      footer={
        ...
        >
          {t("pages.pos.payValidate", { total: formatMoney(total) })}
        </Button>
      }
    >
      <div className="row-between" style={{ marginBottom: 12 }}>
        <span className="muted">{t("pages.pos.toPay")}</span>
        <strong style={{ fontSize: "1.4rem" }}>{formatMoney(total)}</strong>
      </div>

      <Field
        label={t("pages.pos.customerField")}
        hint={t("pages.pos.customerHint")}
      >
        <Select
          value={customerId}
          onChange={(e) => setCustomerId(e.target.value)}
        >
          <option value="">{t("pages.pos.cashOption")}</option>
          {customers.map((c) => (
            <option key={c.id} value={c.id}>
              {c.name}
              {c.balance > 0
                ? t("pages.pos.balanceSuffix", {
      ...
      <Field label={t("pages.pos.methodField")} required>
        <Select
          value={method}
      ...
      {customer ? (
        <>
          <Field
            label={t("pages.pos.paidNowField")}
            hint={t("pages.pos.paidNowHint", { total: formatMoney(total) })}
          >
            <Input
              inputMode="decimal"
              value={paidStr}
          ...
          {credit > 1e-9 ? (
            <>
              <div className="pay-change" style={{ marginBottom: 10 }}>
                <span>{t("pages.pos.leftToPay")}</span>
                <strong>{formatMoney(credit)}</strong>
              </div>
              <Field label={t("pages.pos.dueField")}>
                <Input
                  type="date"
                  value={dueDate}
      ...
      {creditBlocked ? (
        <p style={{ color: "var(--danger)", fontWeight: 600, marginTop: 8 }}>
          {t("pages.pos.creditBlockedWarn")}
        </p>
      ) : null}

      {method === "CASH" ? (
        <>
          <Field label={t("pages.pos.receivedField")}>
            <Input
              inputMode="decimal"
              value={received}
          ...
          {receivedNum != null ? (
            <div className="pay-change">
              <span>{t("pages.pos.changeLabel")}</span>
        ...
        <Field
          label={
            method === "MTN_MOMO" ? t("pages.pos.refMtn") : t("pages.pos.refOm")
          }
          required
          hint={t("pages.pos.refHint")}
        >
          <Input
            value={reference}
...
function SaleSuccess({ sold, onNew }: { sold: SoldState; onNew: () => void }) {
  ...
  const loadReceipt = async (print = false) => {
    if (!sold.saleId) {
      show(t("pages.pos.receiptAfterSync"), "info");
      return;
    }
    try {
  ...
  return (
    <div className="wrap" style={{ maxWidth: 560 }}>
      ...
      >
        ...
        <h1 style={{ margin: 0 }}>
          {sold.offline
            ? t("pages.pos.soldOffline")
            : t("pages.pos.soldOnline")}
        </h1>
        <div className="kpi-value" style={{ color: "var(--primary)" }}>
          {formatMoney(sold.total)}
        </div>
        {sold.offline ? (
          <Badge tone="info">{t("pages.pos.queuedBadge")}</Badge>
        ) : (
          <Badge tone="ok">
            {t("pages.pos.stockDeducted", {
              method: paymentMethodLabel(sold.method),
            })}
          </Badge>
        )}
        {sold.customerName ? (
          <p className="muted" style={{ margin: 0 }}>
            {t("pages.pos.customerLine")} <strong>{sold.customerName}</strong>
          </p>
        ) : null}
        {(sold.outstanding ?? 0) > 0 ? (
          <>
            <Badge tone="warn">{t("pages.pos.creditBadge")}</Badge>
            <p style={{ margin: 0, fontWeight: 700, color: "var(--warn)" }}>
              {t("pages.pos.leftToPayInline", {
                amount: formatMoney(sold.outstanding ?? 0),
              })}
            </p>
            <p className="muted" style={{ margin: 0, fontSize: "0.82rem" }}>
              {t("pages.pos.creditNote", {
                paid: formatMoney(round2(sold.total - (sold.outstanding ?? 0))),
              })}
            </p>
          </>
        ) : null}
        {sold.method === "CASH" && sold.received != null ? (
          <p className="muted" style={{ margin: 0 }}>
            {t("pages.pos.receivedLine", {
              received: formatMoney(sold.received),
            })}{" "}
            <strong>{formatMoney(change)}</strong>
          </p>
        ) : null}
        {sold.reference ? (
          <p className="muted" style={{ margin: 0 }}>
            {t("pages.pos.referenceLine", { ref: sold.reference })}
          </p>
        ) : null}
        <p className="muted" style={{ margin: 0 }}>
...
function SerialPickerModal({
  ...
  const addSerial = (raw: string) => {
    ...
    if (selected.some((x) => x.toLowerCase() === s.toLowerCase())) {
      show(t("pages.pos.serialDuplicate", { serial: s }), "error");
      return;
    }
    ...
    <Modal
      ...
      footer={
        <>
          ...
          >
            {t("pages.pos.serialValidate", { count: selected.length })}
          </Button>
        </>
      }
    >
      <p className="muted" style={{ marginTop: 0 }}>
        {t("pages.pos.serialIntro")}
      </p>
      <Field label={t("pages.pos.serialFieldLabel")}>
        <input
          ref={entryRef}
          className="input"
          value={entry}
          ...
          placeholder={t("pages.pos.serialPlaceholder")}
          autoFocus
        />
      </Field>
      {selected.length > 0 ? (
        <div style={{ margin: "10px 0" }}>
          <strong style={{ fontSize: "0.85rem" }}>
            {t("pages.pos.serialSelectedLabel", { count: selected.length })}
          </strong>
          <div
            className="row"
          ...
          >
            {selected.map((s) => (
              ...
              >
                ...
                <button
                  type="button"
                  aria-label={t("pages.pos.serialRemoveAria", { serial: s })}
                  onClick={() =>
      ...
      {online && available !== null ? (
        <div style={{ marginTop: 12 }}>
          <strong style={{ fontSize: "0.85rem" }}>
            {t("pages.pos.serialAvailableLabel", {
          ...
          {suggestions.length === 0 ? (
            <p className="muted" style={{ fontSize: "0.85rem" }}>
              {available.length === 0
                ? t("pages.pos.serialNoneInStock")
                : t("pages.pos.serialAllPicked")}
            </p>
          ) : (
            <div
              className="row"
