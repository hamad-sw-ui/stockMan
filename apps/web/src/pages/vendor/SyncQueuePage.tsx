...
const statusBadge = (s: OutboxEntry["status"]) =>
  s === "QUEUED" ? (
    <Badge tone="info">En file</Badge>
  ) : s === "SYNCING" ? (
    <Badge>Synchronisation…</Badge>
  ) : (
...
export default function SyncQueuePage() {
  ...
  const syncNow = async () => {
    if (!online) {
      show(
        "Toujours hors-ligne : la synchronisation reprendra automatiquement au retour du réseau.",
        "info",
      );
      return;
    }
    ...
    <div className="wrap" style={{ maxWidth: 760 }}>
      ...
      {entries.length === 0 ? (
        <EmptyState emoji="✨" title="File vide">
          Les ventes passées pendant une coupure réseau s’affichent ici puis
          disparaissent une fois synchronisées.
        </EmptyState>
      ) : (
        <>
          {failed.length > 0 ? (
            <p className="banner banner-warn" style={{ borderRadius: 10 }}>
              ⚠️ {failed.length} vente(s) en échec (refus du serveur, ex. stock
              insuffisant depuis). Vérifiez avec le gérant puis relancez.
            </p>
          ) : null}
          <Card pad={false}>
      ...
      <Card>
        ...
        >
          <li>
            En coupure réseau, chaque vente est conservée localement avec un
            identifiant unique.
          </li>
          <li>
            Au retour du réseau, l’envoi est automatique (et rejoué sans doublon
            possible côté serveur).
          </li>
          <li>
            Le serveur reste l’autorité finale : prix et disponibilité du stock
            sont revérifiés à la synchro.
          </li>
        </ul>
      </Card>
    </div>
