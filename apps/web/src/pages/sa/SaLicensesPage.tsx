...
export default function SaLicensesPage() {
  ...
  const doRenew = async () => {
    ...
    try {
      ...
      show(
        e instanceof Error ? e.message : "Renouvellement impossible",
        "error",
      );
    } finally {
      setBusy(false);
  ...
  return (
    <div className="wrap">
                  ...
                  <tr key={l.id}>
                    ...
                    <td className="num">
                      {l.monthly_price != null
                        ? `${formatMoney(l.monthly_price)}/mois`
                        : "—"}
                    </td>
                    <td>
                      <Button
      ...
      {renew ? (
        <Modal
          title={`Renouveler — ${renew.tenant_name}`}
        ...
        >
          <p className="muted" style={{ marginTop: 0 }}>
            Échéance actuelle : <strong>{formatDate(renew.end_date)}</strong> —
            la prolongation part de cette date (ou d’aujourd’hui si dépassée).
          </p>
          <div className="row">
            ...
            <Field label="Note (paiement reçu…)">
              <Input
                ...
                placeholder="Ex. MoMo reçu le 02/08, réf …"
              />
            </Field>
          </div>
        </Modal>
