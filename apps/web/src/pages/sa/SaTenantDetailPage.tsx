...
export default function SaTenantDetailPage() {
    ...
    try {
      ...
      if (confirm === "reset") {
        ...
        show(
          confirm === "activate"
            ? "Tenant réactivé."
            : "Tenant suspendu (sessions fermées).",
          "success",
        );
        refresh();
  ...
  if (q.loading)
    return (
      <div className="wrap">
        <Spinner label="Chargement du tenant…" />
      </div>
    );
  if (q.error || !q.data)
    return (
  ...
  return (
    <div className="wrap">
      ...
      <div className="kpi-grid">
        ...
        <Kpi
          label="Licence courante"
          value={activeLic ? `${activeLic.plan_code}` : "—"}
          sub={
            activeLic
              ? `fin ${formatDate(activeLic.end_date)} · ${activeLic.status}`
              : "aucune"
          }
        ...
        >
          <Card title="Utilisateurs" pad={false}>
            ...
            <div style={{ padding: 12 }}>
              ...
              >
                🔐 Réinitialiser le mot de passe du gérant
              </Button>
            </div>
          </Card>
                ...
                {t.licenses.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="num muted">
                      {l.max_users} util. · {l.max_depots} dép.
                    </td>
      ...
      {confirm ? (
        <ConfirmModal
          title={
            confirm === "suspend"
              ...
              : confirm === "activate"
                ? "Réactiver le tenant"
                : confirm === "reset"
                  ? "Réinitialiser le mot de passe gérant"
                  : "Connexion support"
          }
          ...
          message={
            confirm === "suspend" ? (
              <>
                Le tenant <strong>{t.name}</strong> sera suspendu : aucun
                utilisateur ne pourra plus se connecter et les sessions actives
                seront fermées. Les données sont conservées.
              </>
            ) : confirm === "activate" ? (
              <>
                Le tenant <strong>{t.name}</strong> redevient actif
                immédiatement.
              </>
            ) : confirm === "reset" ? (
              <>
                Un mot de passe temporaire sera généré pour le gérant de{" "}
                <strong>{t.name}</strong> (affiché une fois) et ses sessions
                seront fermées.
              </>
            ) : (
              <>
                Vous allez ouvrir une session{" "}
                <strong>en tant que gérant</strong> de {t.name}. Cette session
                est <strong>journalisée</strong> et affichée comme « support »
                dans le journal d’audit du tenant.
              </>
            )
          }
          onConfirm={doConfirm}
      ...
      {tempPw ? (
        <Modal
          ...
          footer={
            <Button onClick={() => setTempPw(null)}>J’ai transmis</Button>
          }
        >
          <p>
            Transmettez ce mot de passe au gérant (affiché une seule fois) :
          </p>
          <div
            ...
            >
              {(plans.data ?? []).map((p) => (
                <option key={p.code} value={p.code}>
                  {p.name} — {formatMoney(p.monthly_price)}/mois · {p.max_users}{" "}
                  util. · {p.max_depots} dép.
                </option>
              ))}
            </Select>
          </Field>
