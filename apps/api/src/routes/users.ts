...
const pinSchema = z.string().regex(/^\d{4,6}$/, 'Le PIN comporte 4 à 6 chiffres');

// ============================ LISTE =========================================
router.get(
  '/',
  requireRole('ADMIN'),
  validateQuery(z.object({ role: z.enum(['ADMIN', 'VENDEUR']).optional(), includeInactive: z.coerce.boolean().default(false) })),
  h(async (req, res) => {
    const u = (req as AuthRequest).user;
    const q = req.query as unknown as { role?: string; includeInactive: boolean };
...
async function assertPinAvailable(tenantId: string, pin: string, excludeUserId?: string | null) {
  ...
  for (const row of others.rows) {
    if (await bcrypt.compare(pin, row.pin_hash)) {
      throw HttpError.conflict('PIN_TAKEN', 'Ce PIN est déjà utilisé par un autre compte du tenant.');
    }
  }
}
...
router.post(
  ...
  h(async (req, res) => {
    ...
    if (b.role === 'VENDEUR' && !b.depotId) {
      throw HttpError.badRequest('DEPOT_REQUIRED', 'Un vendeur doit être affecté à un dépôt.');
    }
    const password = b.password ?? crypto.randomBytes(6).toString('base64url') + '1A';
    const env = getEnv();
    ...
    const created = await withTransaction(async (client) => {
      ...
      if (count.rows[0]!.n >= max) {
        throw new HttpError(402, 'LICENSE_LIMIT_USERS', `Votre licence autorise ${max} utilisateur(s) actif(s) maximum.`);
      }
      const hash = await bcrypt.hash(password, env.BCRYPT_ROUNDS);
      const pinHash = b.pin ? await bcrypt.hash(b.pin, env.BCRYPT_ROUNDS) : null;
      const r = await client.query(
        ...
        [u.tenantId, b.name, b.email.toLowerCase(), hash, b.role, b.depotId ?? null, pinHash],
      );
      await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'CREATE', entity: 'user', entityId: r.rows[0]?.id ?? null, newState: r.rows[0] }, client);
      return r.rows[0];
    });
    res.status(201).json({ ...created, generatedPassword: b.password ? undefined : password });
  }),
);
...
router.patch(
  ...
  h(async (req, res) => {
    const u = (req as AuthRequest).user;
    const target = await query('SELECT * FROM users WHERE id=$1 AND tenant_id=$2', [req.params.id!, u.tenantId]);
    if (!target.rows[0]) throw HttpError.notFound('Utilisateur introuvable.');
    const b = req.body;
    if ((b.role ?? target.rows[0].role) === 'VENDEUR' && !(b.depotId ?? target.rows[0].depot_id)) {
      throw HttpError.badRequest('DEPOT_REQUIRED', 'Un vendeur doit être affecté à un dépôt.');
    }
    const r = await query(
      ...
      [req.params.id!, u.tenantId, b.name ?? null, b.email ? b.email.toLowerCase() : null, b.role ?? null, b.depotId ?? null],
    );
    await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'UPDATE', entity: 'user', entityId: req.params.id!, previousState: target.rows[0], newState: r.rows[0] });
    res.json(r.rows[0]);
  }),
);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'UPDATE', entity: 'user', entityId: req.params.id!, details: 'Réinitialisation du mot de passe par un administrateur.' });
    res.json({ message: `Mot de passe réinitialisé pour ${r.email}`, temporaryPassword: temp });
  }),
);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const r = await query('UPDATE users SET pin_hash=$3, updated_at=now() WHERE id=$1 AND tenant_id=$2 RETURNING id', [
      req.params.id!, u.tenantId, hash,
    ]);
    if (!r.rows[0]) throw HttpError.notFound('Utilisateur introuvable.');
    await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'UPDATE', entity: 'user', entityId: req.params.id!, details: 'PIN réinitialisé.' });
...
router.post(
  ...
  h(async (req, res) => {
    const u = (req as AuthRequest).user;
    if (req.params.id === u.id) throw HttpError.badRequest('SELF_DEACTIVATE', 'Impossible de désactiver votre propre compte.');
    ...
    await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'UPDATE', entity: 'user', entityId: req.params.id!, details: 'Compte désactivé.' });
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const r = await query('UPDATE users SET is_active=true, updated_at=now() WHERE id=$1 AND tenant_id=$2 RETURNING email', [
      req.params.id!, u.tenantId,
    ]);
    if (!r.rows[0]) throw HttpError.notFound('Utilisateur introuvable.');
