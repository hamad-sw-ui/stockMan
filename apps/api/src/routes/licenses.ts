...
router.get(
  ...
  h(async (_req, res) => {
    const r = await query(
      "SELECT code, name, max_users, max_depots, monthly_price::float FROM plans ORDER BY monthly_price",
    );
    res.json(r.rows);
  }),
);
...
router.post(
  ...
  h(async (req, res) => {
    const b = req.body;
    const r = await query(
      "INSERT INTO plans (code, name, max_users, max_depots, monthly_price) VALUES ($1,$2,$3,$4,$5) RETURNING *",
...
router.patch(
  ...
  validateBody(
    z.object({
      ...
      maxDepots: z.coerce.number().int().min(1).optional(),
