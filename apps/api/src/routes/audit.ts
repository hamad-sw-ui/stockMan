...
router.get(
  ...
  h(async (req, res) => {
    ...
    const tenantId = u.role === 'SUPER_ADMIN' ? (q.tenantId ?? null) : u.tenantId;
    if (!tenantId) throw HttpError.badRequest('TENANT_REQUIRED', 'Précisez tenantId pour la consultation globale.');
    ...
    const count = await query<{ n: number }>(`SELECT COUNT(*)::int AS n FROM audit_logs a ${where}`, params);
