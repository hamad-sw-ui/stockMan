...
router.post(
  ...
  h(async (req, res) => {
    const session = await authService.loginWithPin(req.body.email, req.body.pin);
    setRefreshCookie(res, session.refreshToken);
    res.json({ accessToken: session.accessToken, user: session.user });
  }),
);
...
router.post(
  '/refresh',
  h(async (req, res) => {
    const session = await authService.refresh(req.cookies?.[authService.refreshCookieConfig.name]);
    setRefreshCookie(res, session.refreshToken);
    res.json({ accessToken: session.accessToken, user: session.user });
  }),
);
...
router.post(
  '/logout',
  h(async (req, res) => {
    await authService.logout(req.cookies?.[authService.refreshCookieConfig.name]);
...
router.post(
  '/reset-password',
  validateBody(z.object({ token: z.string().min(10), newPassword: passwordSchema })),
  h(async (req, res) => {
