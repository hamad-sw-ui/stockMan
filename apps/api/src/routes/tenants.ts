...
router.get(
  ...
  h(async (req, res) => {
    ...
    res.json({
      ...
      usage: { users: users.rows[0]!.n, depots: depots.rows[0]!.n },
    });
  }),
);
...
router.get(
  ...
  requireSuperAdmin,
  validateQuery(
    pageQuerySchema.extend({ search: z.string().trim().default("") }),
  ),
  h(async (req, res) => {
    ...
    const count = await query<{ n: number }>(
      `SELECT COUNT(*)::int AS n FROM tenants t ${joinSearch} ${where}`,
      [q.search],
    );
    ...
    const licenseByTenant = new Map<
      string,
      { planCode: string; status: string; endDate: string | null }
    >();
    if (ids.length > 0) {
      ...
      for (const l of lic.rows) {
        if (!licenseByTenant.has(l.tenant_id)) {
          licenseByTenant.set(l.tenant_id, {
            planCode: l.plan_code,
            status: l.status,
            endDate: toDateStr(l.end_date) ?? l.end_date,
          });
        }
      }
    }
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const created = await withTransaction(async (client) => {
      ...
      if (!plan.rows[0])
        throw HttpError.badRequest("PLAN_UNKNOWN", "Plan inconnu.");
      ...
      await client.query(
        ...
        [
          ...
          status,
          days,
          plan.rows[0]!.max_users,
          plan.rows[0]!.max_depots,
        ],
      );
      ...
      await client.query(
        "INSERT INTO notification_settings (tenant_id) VALUES ($1) ON CONFLICT DO NOTHING",
        [tenantId],
      );
      await client.query(
...
router.get(
  ...
  h(async (req, res) => {
    ...
    const [users, depots, licenses, stats] = await Promise.all([
      query(
        "SELECT id, name, email, role, is_active, created_at FROM users WHERE tenant_id=$1 ORDER BY created_at",
        [req.params.id],
      ),
      query(
        "SELECT id, name, address, is_active FROM depots WHERE tenant_id=$1 ORDER BY name",
        [req.params.id],
      ),
      query(
        "SELECT l.*, p.name AS plan_name FROM licenses l JOIN plans p ON p.code=l.plan_code WHERE l.tenant_id=$1 ORDER BY l.end_date DESC",
        [req.params.id],
      ),
      query(
...
router.patch(
  ...
  validateBody(
    z.object({
      name: z.string().trim().min(2).max(255).optional(),
      phone: z.string().trim().max(50).nullish(),
      subdomain: z.string().trim().max(100).nullish(),
    }),
  ),
  h(async (req, res) => {
    const b = req.body;
  ...
  h(async (req, res) => {
    ...
    const r = await withTransaction(async (client) => {
      const upd = await client.query(
        "UPDATE tenants SET is_active=$2, updated_at=now() WHERE id=$1 RETURNING name, is_active",
    ...
    res.json({
      message: `Tenant « ${r.name} » ${req.body.isActive ? "réactivé" : "suspendu"}.`,
    });
  }),
);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    if (!u)
      throw HttpError.notFound("Aucun administrateur actif sur ce tenant.");
    if (!u.tenant_active)
      throw HttpError.forbidden("Tenant suspendu : impersonation impossible.");
    ...
    await writeAudit({
      tenantId: u.tenant_id,
      userId: sa.id,
      userName: `${sa.name} (support)`,
      action: "IMPERSONATE",
      entity: "user",
      entityId: u.id,
      details: `Session support ouverte par le Super Admin via le compte de ${u.name}.`,
    });
    res.json({
      accessToken: token,
      user: {
...
router.post(
  ...
  h(async (req, res) => {
    ...
    res.json({
      message: `Mot de passe réinitialisé pour ${u.email}`,
