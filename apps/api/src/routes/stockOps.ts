...
import { pageParams, paged, pageQuerySchema, encodeCursor, decodeCursor } from '../lib/pagination';
import { authenticate, AuthRequest, requireRole } from '../middleware/auth';
import { requireActiveLicense } from '../middleware/license';
import { validateBody, validateParams, validateQuery, uuidParam, money, qty } from '../middleware/validate';
import { PoolClient } from 'pg';
import { decreaseLevel, increaseLevel, lockLevel, NO_VARIANT, recordMovement, setLevel } from '../services/stockService';
...
async function unitFactor(client: PoolClient, tenantId: string, productId: string, unitId?: string | null): Promise<{ factor: number; resolvedUnitId: string | null }> {
  const prod = await client.query<{ unit_id: string | null }>('SELECT unit_id FROM products WHERE id=$1 AND tenant_id=$2', [productId, tenantId]);
  if (!prod.rows[0]) throw HttpError.badRequest('PRODUCT_UNKNOWN', 'Produit introuvable.');
  if (!unitId || unitId === prod.rows[0].unit_id) return { factor: 1, resolvedUnitId: unitId ?? prod.rows[0].unit_id };
  const units = await client.query<{ id: string; base_value: string }>(
    ...
    [tenantId, unitId, prod.rows[0].unit_id ?? '00000000-0000-0000-0000-000000000000'],
  );
  ...
  const factor = baseU ? parseFloat(saleU.base_value) / parseFloat(baseU.base_value) : parseFloat(saleU.base_value);
  ...
  h(async (req, res) => {
    ...
    const created = await withTransaction(async (client) => {
      const rec = await client.query<{ id: string }>(
        ...
        [u.tenantId, depotId, b.supplierId ?? null, u.id, b.reference ?? null, b.note ?? null],
      );
      const receiptId = rec.rows[0]!.id;
      let totalCost = 0;

      for (const item of b.items) {
        const { factor } = await unitFactor(client, u.tenantId, item.productId, item.unitId);
        ...
        if (item.batchNumber || item.expiryDate) {
          ...
          if (existing.rows[0]) {
            await client.query('UPDATE stock_batches SET quantity = quantity + $2 WHERE id=$1', [existing.rows[0].id, baseQty]);
            ...
            const batch = await client.query<{ id: string }>(
              ...
              [item.productId, item.variantId ?? null, depotId, b.supplierId ?? null, batchNumber, baseQty, item.expiryDate ?? null],
            );
        ...
        await client.query(
          `INSERT INTO stock_receipt_items (receipt_id, product_id, variant_id, batch_id, base_qty, unit_cost)
           VALUES ($1,$2,$3,$4,$5,$6)`,
          [receiptId, item.productId, item.variantId ?? null, batchId, baseQty, item.unitCost],
        );

        const scope = { tenantId: u.tenantId, depotId, productId: item.productId, variantId: item.variantId ?? null };
        const lvl = await increaseLevel(client, scope, baseQty);
        await recordMovement(client, {
          ...scope, userId: u.id, type: 'IN', quantity: baseQty,
          previousStock: lvl.previous, newStock: lvl.next,
          reason: b.reference ? `Réception ${b.reference}` : 'Réception fournisseur', referenceId: receiptId,
        });
        // Le coût d'achat catalogue suit la dernière réception
        if (item.unitCost > 0) {
          await client.query('UPDATE products SET purchase_price=$2, updated_at=now() WHERE id=$1', [item.productId, item.unitCost]);
        }
      }
      await writeAudit({
        tenantId: u.tenantId, userId: u.id, userName: u.name,
        action: 'RECEIPT', entity: 'receipt', entityId: receiptId, depotId,
...
router.get(
  ...
  h(async (req, res) => {
    ...
    if (q.supplierId) conds.push(`r.supplier_id = $${params.push(q.supplierId)}`);
    if (q.depotId) conds.push(`r.depot_id = $${params.push(q.depotId)}`);
    const where = `WHERE ${conds.join(' AND ')}`;
    const count = await query<{ n: number }>(`SELECT COUNT(*)::int AS n FROM stock_receipts r ${where}`, params);
...
const transferInput = z.object({
  ...
  note: z.string().trim().max(2000).nullish(),
  items: z.array(z.object({ productId: z.string().uuid(), variantId: z.string().uuid().nullish(), quantity: qty })).min(1).max(200),
});
...
router.post(
  ...
  h(async (req, res) => {
    ...
    if (b.fromDepotId === b.toDepotId) throw HttpError.badRequest('SAME_DEPOT', 'Les dépôts source et destination doivent différer.');
    ...
    const created = await withTransaction(async (client) => {
      ...
      if (depots.rows.length !== 2) throw HttpError.badRequest('DEPOT_UNKNOWN', 'Dépôts introuvables ou inactifs.');
      ...
      for (const item of b.items) {
        const scope = { tenantId: u.tenantId, depotId: b.fromDepotId, productId: item.productId, variantId: item.variantId ?? null };
        ...
        await recordMovement(client, {
          ...scope, userId: u.id, type: 'TRANSFER', quantity: item.quantity,
          previousStock: lvl.previous, newStock: lvl.next, reason: 'Transfert sortant', referenceId: transferId,
        });
      }
      await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'TRANSFER', entity: 'transfer', entityId: transferId, newState: { from: b.fromDepotId, to: b.toDepotId, lines: b.items.length } }, client);
...
router.get(
  '/transfers',
  requireRole('ADMIN'),
  validateQuery(pageQuerySchema.extend({ status: z.enum(['PENDING', 'RECEIVED', 'CANCELLED']).optional() })),
  h(async (req, res) => {
    const t = (req as AuthRequest).user.tenantId;
    const q = req.query as unknown as { page: number; size: number; status?: string };
    const { limit, offset } = pageParams(q);
    const params: unknown[] = [t];
    const conds = ['tr.tenant_id = $1'];
    if (q.status) conds.push(`tr.status = $${params.push(q.status)}`);
    const where = `WHERE ${conds.join(' AND ')}`;
    const count = await query<{ n: number }>(`SELECT COUNT(*)::int AS n FROM stock_transfers tr ${where}`, params);
    const rows = await query(
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const done = await withTransaction(async (client) => {
      const tr = await client.query('SELECT * FROM stock_transfers WHERE id=$1 AND tenant_id=$2 FOR UPDATE', [req.params.id!, u.tenantId]);
      const transfer = tr.rows[0];
      if (!transfer) throw HttpError.notFound('Transfert introuvable.');
      if (transfer.status !== 'PENDING') throw HttpError.conflict('TRANSFER_CLOSED', 'Ce transfert est déjà clôturé.');
      const items = await client.query('SELECT * FROM stock_transfer_items WHERE transfer_id=$1', [transfer.id]);
      for (const item of items.rows) {
        const scope = { tenantId: u.tenantId, depotId: transfer.to_depot, productId: item.product_id, variantId: item.variant_id };
        const lvl = await increaseLevel(client, scope, parseFloat(item.quantity));
        await recordMovement(client, {
          ...scope, userId: u.id, type: 'TRANSFER', quantity: parseFloat(item.quantity),
          previousStock: lvl.previous, newStock: lvl.next, reason: 'Transfert reçu', referenceId: transfer.id,
        });
      }
      await client.query("UPDATE stock_transfers SET status='RECEIVED', received_by=$2, received_at=now() WHERE id=$1", [transfer.id, u.id]);
      await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'TRANSFER', entity: 'transfer', entityId: transfer.id, previousState: { status: 'PENDING' }, newState: { status: 'RECEIVED' } }, client);
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const done = await withTransaction(async (client) => {
      ...
      const items = await client.query('SELECT * FROM stock_transfer_items WHERE transfer_id=$1', [transfer.id]);
      for (const item of items.rows) {
        const scope = { tenantId: u.tenantId, depotId: transfer.from_depot, productId: item.product_id, variantId: item.variant_id };
        const lvl = await increaseLevel(client, scope, parseFloat(item.quantity));
        await recordMovement(client, {
          ...scope, userId: u.id, type: 'TRANSFER', quantity: parseFloat(item.quantity),
          previousStock: lvl.previous, newStock: lvl.next, reason: 'Annulation de transfert', referenceId: transfer.id,
        });
      }
      await client.query("UPDATE stock_transfers SET status='CANCELLED' WHERE id=$1", [transfer.id]);
      await writeAudit({ tenantId: u.tenantId, userId: u.id, userName: u.name, action: 'TRANSFER', entity: 'transfer', entityId: transfer.id, previousState: { status: 'PENDING' }, newState: { status: 'CANCELLED' } }, client);
...
const adjustInput = z
  .object({
    ...
    reason: z.string().trim().min(3, 'Un motif détaillé est obligatoire').max(2000),
  })
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const result = await withTransaction(async (client) => {
      const scope = { tenantId: u.tenantId, depotId, productId: b.productId, variantId: b.variantId ?? null };
      const previous = await lockLevel(client, scope);
      const next = b.newQuantity !== undefined ? b.newQuantity : round2(previous + (b.delta ?? 0));
      if (next < 0) throw HttpError.badRequest('STOCK_NEGATIVE', 'Le stock ne peut pas devenir négatif.');
      ...
      if (b.type === 'EXPIRED' && next < previous) {
        ...
        for (const batch of batches.rows) {
          ...
          await client.query('UPDATE stock_batches SET quantity=quantity-$2 WHERE id=$1', [batch.id, take]);
          toClear -= take;
        }
      }

      await recordMovement(client, {
        ...scope, userId: u.id, type: b.type, quantity: Math.abs(round2(next - previous)),
        previousStock: previous, newStock: next, reason: b.reason,
      });
      await writeAudit({
        tenantId: u.tenantId, userId: u.id, userName: u.name,
        action: 'ADJUST', entity: 'stock', entityId: b.productId, depotId,
        previousState: { quantity: previous }, newState: { quantity: next, type: b.type }, details: b.reason,
      }, client);
...
const movementQuery = z.object({
  ...
  type: z.enum(['IN', 'OUT', 'TRANSFER', 'ADJUSTMENT', 'SALE', 'RETURN', 'DAMAGE', 'EXPIRED', 'VOID']).optional(),
...
router.get(
  ...
  h(async (req, res) => {
    ...
    if (c) {
      ...
      conds.push(`(sm.created_at < $${params.push(c.createdAt)} OR (sm.created_at = $${params.length} AND sm.id < $${params.push(c.id)}))`);
    }
