...
async function seed() {
  ...
  try {
    ...
    const piece = await client.query(`SELECT id FROM units WHERE tenant_id=$1 AND symbol='Pce'`, [TENANT_ID]);
    const boissons = await client.query(`SELECT id FROM categories WHERE tenant_id=$1 AND name='Boissons'`, [TENANT_ID]);
    const alim = await client.query(`SELECT id FROM categories WHERE tenant_id=$1 AND name='Alimentaire'`, [TENANT_ID]);
    const unitId = piece.rows[0].id;

    const products: Array<[string, string, number, number, number, string, number]> = [
      ['Eau Tangui 1.5L', boissons.rows[0].id, 250, 400, 20, '6111250900015', 150],
      ['Jus Top Pamplemousse 50cl', boissons.rows[0].id, 200, 350, 10, '6111250900022', 4],
    ...
    for (const [name, catId, pa, pv, seuil, barcode, qty] of products) {
      ...
      await client.query(
        `INSERT INTO stock_levels (product_id, depot_id, quantity) VALUES ($1,$2,$3)
         ON CONFLICT (product_id, depot_id, variant_id) DO NOTHING`,
        [p.rows[0].id, DEPOT_A, qty],
      );
    }

    await client.query('COMMIT');
    logger.info('✅ Seed dev terminé — admin@demo.cm / vendeur@demo.cm (Demo1234!, PIN 4321) · sa@stockman.cm');
  } catch (err) {
...
seed()
  .catch((err) => {
    logger.error('Seed dev échoué', { message: err instanceof Error ? err.message : String(err) });
    process.exitCode = 1;
  })
  .finally(() => closePool());
