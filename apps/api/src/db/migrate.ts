...
async function main() {
  ...
  logger.info('Migrations terminées', { applied: applied.length, skipped: skipped.length });
}

main()
  .catch((err) => {
    logger.error('Migration échouée', { message: err instanceof Error ? err.message : String(err) });
    process.exitCode = 1;
  })
  .finally(() => closePool());
