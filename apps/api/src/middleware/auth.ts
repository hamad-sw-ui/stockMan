...
export function authenticate(req: Request, _res: Response, next: NextFunction) {
  ...
  try {
    const decoded = jwt.verify(token, getEnv().JWT_SECRET) as AccessTokenPayload & jwt.JwtPayload;
    ...
    return next(HttpError.unauthorized('Session expirée ou jeton invalide.', 'TOKEN_INVALID'));
  }
}
...
export function requireRole(...roles: Role[]) {
  return (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (!roles.includes(user.role)) {
      return next(HttpError.forbidden('Accès réservé aux rôles : ' + roles.join(', ')));
    }
    return next();
  };
}
