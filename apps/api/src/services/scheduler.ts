...
import { sendDailyReport, sendExpiryAlerts, sendLowStockAlerts } from './notificationService';
...
async function withLock<T>(fn: () => Promise<T>): Promise<T | null> {
  ...
  try {
    ...
    await query('SELECT pg_advisory_unlock($1)', [LOCK_ID]).catch(() => undefined);
  }
}
...
async function forEachActiveTenant(fn: (tenantId: string) => Promise<void>) {
  ...
  for (const t of tenants.rows) {
    try {
      ...
      logger.error('Job tenant échoué', { tenantId: t.id, message: err instanceof Error ? err.message : String(err) });
    }
  }
}
...
export function startScheduler(): void {
  ...
  cron.schedule('*/5 * * * *', () => {
    void withLock(async () => {
      const rows = await query<{ tenant_id: string; daily_report_time: string; daily_report_enabled: boolean; timezone?: string }>(
      ...
      for (const row of rows.rows) {
        ...
        const local = new Date().toLocaleTimeString('fr-FR', { timeZone: confT, hour12: false });
        const [wantH, wantM] = String(row.daily_report_time).split(':').map(Number);
        const [nowH, nowM] = local.split(':').map(Number);
        const want = (wantH ?? 20) * 60 + (wantM ?? 0);
        const now = (nowH ?? 0) * 60 + (nowM ?? 0);
        const dateKey = new Date().toLocaleDateString('fr-CA', { timeZone: confT });
        const marker = `${row.tenant_id}:${dateKey}`;
        if (Math.abs(now - want) <= 4 && lastDailyCheck[row.tenant_id] !== marker) {
          lastDailyCheck[row.tenant_id] = marker;
          await sendDailyReport(row.tenant_id).catch((err) =>
            logger.error('Rapport journalier échoué', { tenantId: row.tenant_id, message: String(err) }),
          );
        }
      }
    });
