...
import { decreaseLevel, fefoConsume, recordMovement, restock, StockScope } from './stockService';
...
export function resolveDepot(user: AuthUser, requested?: string): string {
  const depot = user.role === 'VENDEUR' ? user.depotId : (requested ?? user.depotId);
  if (!depot) throw HttpError.badRequest('DEPOT_REQUIRED', 'Dépôt requis pour cette opération.');
  if (user.role === 'VENDEUR' && requested && requested !== user.depotId) {
    throw HttpError.forbidden('Un vendeur ne peut opérer que sur son propre dépôt.', 'DEPOT_FORBIDDEN');
  }
  return depot;
}
...
async function loadUnit(client: PoolClient, tenantId: string, unitId: string) {
  const r = await client.query<{ id: string; base_value: string; symbol: string }>(
    'SELECT id, base_value, symbol FROM units WHERE id=$1 AND tenant_id=$2',
    [unitId, tenantId],
  );
  const u = r.rows[0];
  if (!u) throw HttpError.badRequest('UNIT_UNKNOWN', 'Unité inconnue pour ce tenant.');
  return u;
}

async function fullSaleById(client: PoolClient, tenantId: string, saleId: string) {
...
export async function createSale(user: AuthUser, input: CreateSaleInput) {
  ...
  if (input.clientSaleId) {
    const dup = await query('SELECT id FROM sales WHERE tenant_id=$1 AND client_sale_id=$2', [
      user.tenantId, input.clientSaleId,
    ]);
    if (dup.rows[0]) {
      const sale = await withTransaction((c) => fullSaleById(c, user.tenantId, dup.rows[0]!.id));
  ...
  if (input.createdAt) {
    ...
    if (Number.isNaN(ts.getTime())) throw HttpError.badRequest('DATE_INVALID', 'Date de vente invalide.');
    const ageH = (Date.now() - ts.getTime()) / 3600000;
    if (ageH < -0.25) throw HttpError.badRequest('DATE_FUTURE', 'Une vente ne peut pas être datée dans le futur.');
    if (ageH > env.MAX_SYNC_AGE_HOURS) {
      throw HttpError.badRequest('SALE_TOO_OLD', `Vente trop ancienne pour la synchronisation (> ${env.MAX_SYNC_AGE_HOURS} h).`);
    }
  }
  ...
  return withTransaction(async (client) => {
    ...
    if (!depotCheck.rows[0]) throw HttpError.badRequest('DEPOT_UNKNOWN', 'Dépôt introuvable.');
    if (!depotCheck.rows[0].is_active) throw HttpError.badRequest('DEPOT_INACTIVE', 'Ce dépôt est désactivé.');

    let total = 0;
    const prepared: Array<{
      product: ProductRow; variantId: string | null; unitId: string | null;
      unitSymbol: string | null; quantity: number; baseQty: number;
    ...
    for (const item of input.items) {
      ...
      if (!product || product.archived_at) {
        throw HttpError.badRequest('PRODUCT_UNKNOWN', `Produit introuvable ou archivé (${item.productId}).`);
      }
      ...
      if (item.variantId) {
        ...
        if (!vr.rows[0]) throw HttpError.badRequest('VARIANT_UNKNOWN', `Variante introuvable pour « ${product.name} ».`);
      ...
      const productUnitValue = product.unit_id ? parseFloat((await loadUnit(client, user.tenantId, product.unit_id)).base_value) : 1;
      ...
      if (baseQty <= 0) throw HttpError.badRequest('QUANTITY_INVALID', 'Quantité convertie invalide.');

      const discount = Math.min(Math.max(item.discountPct ?? 0, 0), 100);
      const unitPrice = round2((parseFloat(product.selling_price) + additional) * factor * (1 - discount / 100));
      const lineTotal = round2(item.quantity * factor * (parseFloat(product.selling_price) + additional) * (1 - discount / 100));
      total = round2(total + lineTotal);

      prepared.push({ product, variantId, unitId, unitSymbol, quantity: item.quantity, baseQty, unitPrice, lineTotal });
    }
    ...
    const saleIns = await client.query<{ id: string }>(
      ...
      [user.tenantId, depotId, user.id, total, input.paymentMethod, input.paymentReference ?? null,
       input.clientSaleId ?? null, input.createdAt ?? null],
    );
    const saleId = saleIns.rows[0]!.id;

    for (const p of prepared) {
      const scope: StockScope = { tenantId: user.tenantId, depotId, productId: p.product.id, variantId: p.variantId };
      ...
      await client.query(
        ...
        [saleId, p.product.id, p.variantId, p.unitId, p.quantity, p.baseQty, p.unitPrice, p.lineTotal],
      );
      await recordMovement(client, {
        ...scope, userId: user.id, type: 'SALE', quantity: p.baseQty,
        previousStock: lvl.previous, newStock: lvl.next, referenceId: saleId,
      });
    }

    await writeAudit({
      tenantId: user.tenantId, userId: user.id, userName: user.name,
      action: 'SALE', entity: 'sale', entityId: saleId, depotId,
      newState: { total, paymentMethod: input.paymentMethod, items: prepared.length },
    }, client);
...
export async function voidSale(user: AuthUser, saleId: string, reason?: string) {
  return withTransaction(async (client) => {
    const sale = await fullSaleById(client, user.tenantId, saleId);
    if (sale.status === 'VOIDED') throw HttpError.conflict('ALREADY_VOIDED', 'Cette vente est déjà annulée.');

    const sameDay = new Date(sale.created_at).toDateString() === new Date().toDateString();
    if (!sameDay && user.role !== 'SUPER_ADMIN') {
      throw HttpError.forbidden("Annulation limitée au jour même de la vente.", 'VOID_WINDOW');
    }

    await client.query("UPDATE sales SET status='VOIDED' WHERE id=$1", [saleId]);
    for (const item of sale.items) {
      const scope: StockScope = { tenantId: user.tenantId, depotId: sale.depot_id, productId: item.product_id, variantId: item.variant_id };
      const lvl = await restock(client, scope, parseFloat(item.base_qty));
      await recordMovement(client, {
        ...scope, userId: user.id, type: 'VOID', quantity: parseFloat(item.base_qty),
        previousStock: lvl.previous, newStock: lvl.next,
        reason: reason ?? 'Annulation de vente', referenceId: saleId,
      });
    }
    await writeAudit({
      tenantId: user.tenantId, userId: user.id, userName: user.name,
      action: 'VOID', entity: 'sale', entityId: saleId, depotId: sale.depot_id,
      previousState: { status: 'COMPLETED', total: sale.total_amount },
      newState: { status: 'VOIDED' }, details: reason,
    }, client);
...
export async function returnSaleItems(
  ...
  return withTransaction(async (client) => {
    const sale = await fullSaleById(client, user.tenantId, saleId);
    if (sale.status === 'VOIDED') throw HttpError.conflict('SALE_VOIDED', 'Impossible de retourner une vente annulée.');

    // Quantités déjà retournées (aggrégat en 2 requêtes : pas de SUM sur jointure)
    const priorReturns = await client.query<{ id: string }>('SELECT id FROM sale_returns WHERE sale_id=$1', [saleId]);
    const returnedByProduct = new Map<string, number>();
    if (priorReturns.rows.length > 0) {
      const ph = priorReturns.rows.map((_, i) => `$${i + 1}`).join(',');
      const priorItems = await client.query<{ product_id: string; variant_id: string | null; base_qty: string }>(
      ...
      for (const row of priorItems.rows) {
        ...
        returnedByProduct.set(key, (returnedByProduct.get(key) ?? 0) + parseFloat(row.base_qty));
      }
    }

    // 1) VALIDATION INTÉGRALE AVANT TOUTE ÉCRITURE (aucune ligne partielle)
    interface PreparedReturn { line: (typeof sale.items)[number]; baseQty: number; unitPrice: number }
    const prepared: PreparedReturn[] = [];
    let refundedTotal = 0;
    for (const it of items) {
      const line = sale.items.find((i: { id: string }) => i.id === it.saleItemId);
      if (!line) throw HttpError.badRequest('LINE_UNKNOWN', 'Ligne de vente introuvable.');
      ...
      if (alreadyReturned + it.baseQty > parseFloat(line.base_qty) + 1e-9) {
        throw HttpError.badRequest('RETURN_EXCEEDS', `Retour supérieur à la quantité vendue pour « ${line.product_name} ».`);
      }
    ...
    for (const p of prepared) {
      await client.query(
        `INSERT INTO sale_return_items (return_id, product_id, variant_id, base_qty, unit_price)
         VALUES ($1,$2,$3,$4,$5)`,
        [returnId, p.line.product_id, p.line.variant_id, p.baseQty, p.unitPrice],
      );
      const scope: StockScope = { tenantId: user.tenantId, depotId: sale.depot_id, productId: p.line.product_id, variantId: p.line.variant_id };
      const lvl = await restock(client, scope, p.baseQty);
      await recordMovement(client, {
        ...scope, userId: user.id, type: 'RETURN', quantity: p.baseQty,
        previousStock: lvl.previous, newStock: lvl.next,
        reason: reason ?? 'Retour client', referenceId: saleId,
      });
    }
    await writeAudit({
      tenantId: user.tenantId, userId: user.id, userName: user.name,
      action: 'RETURN', entity: 'sale', entityId: saleId, depotId: sale.depot_id,
      newState: { returnId, refundedTotal },
    }, client);
    return { returnId, refundedTotal, sale: await fullSaleById(client, user.tenantId, saleId) };
  });
}
...
export async function receiptData(tenantId: string, saleId: string) {
  ...
  const lines = sale.items.map((i: { product_name: string; variant_name: string | null; quantity: string; unit_symbol: string | null; unit_price: string; total_price: string }) => ({
  ...
  const text = [
    ...
    '--------------------------------',
    ...lines.map((l: { label: string; qty: number; unit: string; total: number }) => `${l.label}\n  ${l.qty} ${l.unit}  —  ${l.total.toLocaleString('fr-FR')} ${tenant.currency}`),
    '--------------------------------',
