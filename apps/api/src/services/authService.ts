...
function accessPayload(u: DbUser, impersonated = false): AccessTokenPayload {
  return { id: u.id, tenantId: u.tenant_id, role: u.role, name: u.name, depotId: u.depot_id, imp: impersonated || undefined };
}

async function issueRefreshToken(userId: string, tenantId: string): Promise<string> {
...
function assertCanLogin(u: DbUser) {
  if (!u.is_active) throw HttpError.forbidden('Ce compte est désactivé. Contactez votre administrateur.', 'USER_DISABLED');
  if (!u.tenant_active) throw HttpError.forbidden("L'abonnement de cette entreprise est suspendu.", 'TENANT_DISABLED');
}
...
export async function login(email: string, password: string) {
  ...
  if (!ok) throw HttpError.unauthorized('Identifiants invalides.', 'BAD_CREDENTIALS');
  assertCanLogin(user!);
  await writeAudit({ tenantId: user!.tenant_id, userId: user!.id, userName: user!.name, action: 'LOGIN', entity: 'session', entityId: user!.id });
  return buildSession(user!);
}

export async function loginWithPin(email: string, pin: string) {
  const user = await findUserByEmail(email);
  if (!user?.pin_hash) throw HttpError.unauthorized('Identifiants invalides.', 'BAD_CREDENTIALS');
  const ok = await bcrypt.compare(pin, user.pin_hash);
  if (!ok) throw HttpError.unauthorized('Identifiants invalides.', 'BAD_CREDENTIALS');
  assertCanLogin(user);
  await writeAudit({ tenantId: user.tenant_id, userId: user.id, userName: user.name, action: 'LOGIN', entity: 'session', details: 'Connexion PIN' });
  return buildSession(user);
}

// ---------------------------------------------------------------------------
export async function refresh(rawToken: string | undefined) {
  if (!rawToken) throw HttpError.unauthorized('Session absente.', 'NO_REFRESH_TOKEN');
  const tokenHash = sha256(rawToken);
  const r = await query<{ id: string; user_id: string; revoked_at: string | null; replaced_by: string | null; expires_at: string }>(
    `SELECT id, user_id, revoked_at, replaced_by, expires_at FROM refresh_tokens WHERE token_hash = $1`,
    [tokenHash],
  );
  const row = r.rows[0];
  if (!row) throw HttpError.unauthorized('Session invalide.', 'REFRESH_UNKNOWN');
  ...
  if (row.revoked_at) {
    ...
    throw HttpError.unauthorized('Session compromise, veuillez vous reconnecter.', 'REFRESH_REUSE');
  }
...
export async function logout(rawToken: string | undefined, userId?: string) {
  if (rawToken) {
    await query('UPDATE refresh_tokens SET revoked_at = now() WHERE token_hash = $1', [sha256(rawToken)]);
  } else if (userId) {
...
export async function forgotPassword(email: string): Promise<{ devToken?: string }> {
  ...
  const token = jwt.sign({ sub: user.id, purpose: RESET_PURPOSE }, getEnv().REFRESH_SECRET, { expiresIn: '30m' });
...
export async function resetPassword(token: string, newPassword: string) {
  ...
  try {
    payload = jwt.verify(token, getEnv().REFRESH_SECRET) as { sub: string; purpose: string };
  } catch {
    throw HttpError.badRequest('RESET_TOKEN_INVALID', 'Lien de réinitialisation expiré ou invalide.');
  }
  ...
  await withTransaction(async (client) => {
    const r = await client.query('UPDATE users SET password_hash = $2, updated_at = now() WHERE id = $1 RETURNING tenant_id', [
      payload.sub,
      hash,
    ]);
    if (r.rows.length === 0) throw HttpError.notFound('Compte introuvable.');
    await client.query('UPDATE refresh_tokens SET revoked_at = now() WHERE user_id = $1 AND revoked_at IS NULL', [payload.sub]);
  });
}
...
export async function changePassword(userId: string, current: string, next: string) {
  ...
  if (!ok) throw HttpError.badRequest('BAD_CURRENT_PASSWORD', 'Mot de passe actuel incorrect.');
...
export async function me(userId: string) {
  ...
  return {
    ...publicUser(user),
    license: lic ? { ...lic, end_date: toDateStr(lic.end_date) ?? lic.end_date } : null,
  };
}
