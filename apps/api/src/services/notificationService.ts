...
const africaTalkingProvider = (): SmsProvider => ({
  ...
  async send(to, message) {
    const env = getEnv();
    if (!env.AT_API_KEY || !env.AT_USERNAME) throw new Error("Clés Africa's Talking absentes");
    ...
    const recipients = (data.SMSMessageData as { Recipients?: Array<{ status: string }> } | undefined)?.Recipients ?? [];
...
const whatsappProvider = (): SmsProvider => ({
  ...
  async send(to, message) {
    const env = getEnv();
    if (!env.WA_TOKEN || !env.WA_PHONE_ID) throw new Error('Clés WhatsApp absentes');
 ...
 */
export async function notify(input: NotifyInput): Promise<'sent' | 'failed' | 'deduped'> {
  try {
    const ins = await query<{ id: string }>(
      `INSERT INTO notifications (tenant_id, user_id, phone, type, channel, message, status, dedupe_key)
       ...
       RETURNING id`,
      [input.tenantId, input.userId ?? null, input.phone ?? null, input.type, input.channel, input.message, input.dedupeKey ?? null],
    );
    const id = ins.rows[0]?.id;
    if (!id) return 'deduped';

    if (input.channel === 'IN_APP') {
      await query(`UPDATE notifications SET status='SENT', provider_response=$2 WHERE id=$1`, [id, JSON.stringify({ inApp: true })]);
      return 'sent';
    }
    if (!input.phone) {
      await query(`UPDATE notifications SET status='FAILED', provider_response=$2 WHERE id=$1`, [id, JSON.stringify({ reason: 'destinataire absent' })]);
      return 'failed';
    }

    try {
      const result = await pickProvider(input.channel).send(input.phone, input.message);
      await query('UPDATE notifications SET status=$2, provider_response=$3 WHERE id=$1', [
        id, result.ok ? 'SENT' : 'FAILED', JSON.stringify(result.detail),
      ]);
      return result.ok ? 'sent' : 'failed';
    } catch (err) {
      await query('UPDATE notifications SET status=$2, provider_response=$3 WHERE id=$1', [
        id, 'FAILED', JSON.stringify({ error: err instanceof Error ? err.message : String(err) }),
      ]);
      return 'failed';
    }
  } catch (err) {
    logger.error('notify() a échoué', { message: err instanceof Error ? err.message : String(err) });
    return 'failed';
  }
}
...
export async function sendLowStockAlerts(tenantId: string): Promise<void> {
  ...
  const all = await query<{ name: string; total: number; min_stock_level: number }>(
  ...
  const list = low
    .map((p) => `- ${p.name} : ${p.total} restant(s) (seuil ${p.min_stock_level})`)
    .join('\n');
  const message = `⚠️ Alerte stock bas — ${low.length} produit(s) sous le seuil :\n${list}\nConnectez-vous à StockMan pour réapprovisionner.`;

  await notify({ tenantId, channel: 'IN_APP', type: 'LOW_STOCK', message, dedupeKey: `LOW_STOCK:${todayKey()}` });
  if (cfg.alert_phone) {
    await notify({ tenantId, channel: 'SMS', phone: cfg.alert_phone, type: 'LOW_STOCK', message, dedupeKey: `LOW_STOCK_SMS:${todayKey()}` });
  }
  if (cfg.alert_whatsapp) {
    await notify({ tenantId, channel: 'WHATSAPP', phone: cfg.alert_whatsapp, type: 'LOW_STOCK', message, dedupeKey: `LOW_STOCK_WA:${todayKey()}` });
  }
}
...
export async function sendExpiryAlerts(tenantId: string): Promise<void> {
  ...
  const soon = await query<{ name: string; batch_number: string; expiry_date: Date; quantity: string }>(
  ...
  const list = soon.rows
    .map((r) => `- ${r.name} (lot ${r.batch_number}) : ${parseFloat(String(r.quantity))} u. expire le ${toDateStr(r.expiry_date)}`)
    .join('\n');
  const message = `⏳ Péremptions proches (≤ 7 j) :\n${list}`;
  await notify({ tenantId, channel: 'IN_APP', type: 'EXPIRY', message, dedupeKey: `EXPIRY:${todayKey()}` });
  if (cfg.alert_whatsapp) {
    await notify({ tenantId, channel: 'WHATSAPP', phone: cfg.alert_whatsapp, type: 'EXPIRY', message, dedupeKey: `EXPIRY_WA:${todayKey()}` });
  }
}
...
export async function sendDailyReport(tenantId: string): Promise<void> {
  ...
  const top = topRows.rows.map((t) => `${t.name} ×${parseFloat(String(t.qty))}`).join(', ');
  const message =
    `📊 Rapport du jour :\n` +
    `• Chiffre d'affaires : ${parseFloat(String(r.revenue)).toLocaleString('fr-FR')} FCFA\n` +
    `• Ventes : ${r.count}\n` +
    (top ? `• Top produits : ${top}` : '• Aucune vente aujourd’hui.');
  await notify({ tenantId, channel: 'IN_APP', type: 'DAILY_REPORT', message, dedupeKey: `DAILY:${todayKey()}` });
  if (cfg.alert_whatsapp) {
    await notify({ tenantId, channel: 'WHATSAPP', phone: cfg.alert_whatsapp, type: 'DAILY_REPORT', message, dedupeKey: `DAILY_WA:${todayKey()}` });
  } else if (cfg.alert_phone) {
    await notify({ tenantId, channel: 'SMS', phone: cfg.alert_phone, type: 'DAILY_REPORT', message, dedupeKey: `DAILY_SMS:${todayKey()}` });
  }
}
