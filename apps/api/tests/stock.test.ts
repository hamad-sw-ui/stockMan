...
describe("Opérations de stock (réceptions, transferts, ajustements, FEFO)", () => {
  it("réception : crée le mouvement IN, le lot et met à jour le coût d’achat", async () => {
    ...
    expect(
      detail.body.batches.some(
        (b: { batch_number: string }) => b.batch_number === "LOT-001",
      ),
    ).toBe(true);
  ...
  it("transfert > stock disponible : refusé sans effet de bord", async () => {
    ...
    const res = await ctx.agent
      ...
      .send({
        fromDepotId: ids.depotId,
        toDepotId: depotB,
        items: [{ productId: ids.productId, quantity: 999999 }],
      });
  ...
  it("annulation de transfert : restitution au dépôt d’origine", async () => {
    ...
    await ctx.agent
      .post(`/api/stock/transfers/${create.body.transferId}/cancel`)
      .set(auth(ids.adminToken));
    expect(await qtyOf(ids.productId, ids.depotId)).toBe(fromBefore);
  ...
  it("ajustement : motif obligatoire, mouvement tracé, types DAMAGE/EXPIRED", async () => {
    ...
    const ok = await ctx.agent
      ...
      .send({
        ...
        type: "DAMAGE",
        delta: -3,
        reason: "Casier renversé, 3 bouteilles cassées",
      });
    ...
    const moves = await ctx.agent
      .get(`/api/stock/movements?productId=${ids.productId}&type=DAMAGE`)
      .set(auth(ids.adminToken));
  ...
  it("FEFO : la vente consomme le lot qui expire le plus tôt, jamais un lot périmé", async () => {
    ...
    const sale = await ctx.agent
      ...
      .send({
        items: [{ productId: pid, quantity: 8 }],
        paymentMethod: "CASH",
      });
    ...
    const batches = Object.fromEntries(
      detail.body.batches.map(
        (b: { batch_number: string; quantity: string }) => [
          b.batch_number,
    ...
    const tooMuch = await ctx.agent
      ...
      .set(auth(ids.vendorToken))
      .send({
        items: [{ productId: pid, quantity: 13 }],
        paymentMethod: "CASH",
      });
  ...
  it("mouvements : pagination par curseur stable", async () => {
    const page1 = await ctx.agent
      .get("/api/stock/movements?size=5")
      .set(auth(ids.adminToken));
    expect(page1.body.data.length).toBe(5);
    expect(page1.body.nextCursor).toBeTruthy();
    const page2 = await ctx.agent
      .get(
        `/api/stock/movements?size=5&cursor=${encodeURIComponent(page1.body.nextCursor)}`,
      )
      .set(auth(ids.adminToken));
