...
beforeAll(async () => {
  ...
  await ctx.agent.post('/api/sales').set(v).send({ items: [{ productId: ids.productId, quantity: 3 }], paymentMethod: 'CASH' });
  await ctx.agent.post('/api/sales').set(v).send({ items: [{ productId: ids.productId, unitId: ids.cartonId, quantity: 2 }], paymentMethod: 'MTN_MOMO', paymentReference: 'MOMO-RPT-1' });
  // Une vente annulée (exclue du CA, comptée dans les avoirs)
  const test = await ctx.agent.post('/api/sales').set(v).send({ items: [{ productId: ids.productId, quantity: 1 }], paymentMethod: 'CASH' });
  await ctx.agent.post(`/api/sales/${test.body.sale.id}/void`).set(auth(ids.adminToken)).send({ reason: 'Test' });
  // Un lot périssable à J+5 (stock total 83)
  await receiveStock(ctx, ids, 10, { batchNumber: 'EXP-5', expiryDate: inDays(5) });
  // Le seuil d'alerte passe à 200 pour les vues prédictives
  await ctx.agent.patch(`/api/products/${ids.productId}`).set(auth(ids.adminToken)).send({ minStockLevel: 200 });
}, 90_000);
afterAll(async () => {
...
describe('Rapports décisionnels — exactitude des chiffres (BCK-02, DAT-08)', () => {
  it('tableau de bord : CA, série complétée sans trou, top produits, mix paiement', async () => {
    const res = await ctx.agent.get('/api/reports/dashboard').set(auth(ids.adminToken));
    ...
    const dayRow = res.body.series.find((d: { date: string }) => d.date === today) ?? res.body.series[res.body.series.length - 1];
    ...
    const momo = res.body.paymentMix.find((m: { payment_method: string }) => m.payment_method === 'MTN_MOMO');
    expect(momo.amount).toBe(9600);
    const cash = res.body.paymentMix.find((m: { payment_method: string }) => m.payment_method === 'CASH');
  ...
  it('rapport des ventes par jour/dépôt/vendeur + export CSV', async () => {
    const res = await ctx.agent.get('/api/reports/sales').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const total = res.body.data.reduce((acc: number, r: { revenue: number }) => acc + Number(r.revenue), 0);
    expect(total).toBe(10800);
    expect(res.body.data[0].vendor).toBe('Vendeur Test');
    expect(res.body.data[0].depot).toBe('Dépôt Principal');

    const csv = await ctx.agent.get('/api/reports/sales?format=csv').set(auth(ids.adminToken));
  ...
  it('marges : coût et taux calculés côté serveur', async () => {
    const res = await ctx.agent.get('/api/reports/margin').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const row = res.body.data.find((r: { name: string }) => r.name === 'Eau Test 1.5L');
  ...
  it('valorisation du stock : quantités et valeurs exactes', async () => {
    const res = await ctx.agent.get('/api/reports/stock-valuation').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const row = res.body.data.find((r: { product: string }) => r.product === 'Eau Test 1.5L');
  ...
  it('péremptions : le lot à J+5 apparaît avec son compte à rebours', async () => {
    const res = await ctx.agent.get('/api/reports/expiry?days=30').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const row = res.body.find((r: { batch_number: string }) => r.batch_number === 'EXP-5');
  ...
  it('prédictif : vitesse de vente et horizon de rupture calculés', async () => {
    const res = await ctx.agent.get('/api/reports/predictive').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const row = res.body.find((r: { name: string }) => r.name === 'Eau Test 1.5L');
    expect(row).toBeTruthy();
  ...
  it('Z de caisse : clôture du jour avec avoirs', async () => {
    const res = await ctx.agent.get('/api/reports/z-report').set(auth(ids.adminToken));
    expect(res.status).toBe(200);
    const depot = res.body.totals.find((t: { depot: string }) => t.depot === 'Dépôt Principal');
    ...
    const vendorZ = await ctx.agent.get('/api/reports/z-report').set(auth(ids.vendorToken));
  ...
  it('stats plateforme (Super Admin)', async () => {
    ...
    const saTenant = await ctx.pool.query("INSERT INTO tenants (name) VALUES ('Éditeur SA') RETURNING id");
    ...
    const saLogin = await ctx.agent.post('/api/auth/login').send({ email: 'sa-rpt@test.cm', password: 'Sa123456!' });
    const denied = await ctx.agent.get('/api/reports/superadmin/stats').set(auth(ids.adminToken));
    expect(denied.status).toBe(403);

    const res = await ctx.agent.get('/api/reports/superadmin/stats').set(auth(saLogin.body.accessToken));
    expect(res.status).toBe(200);
...
describe('Caisse (POS) — bootstrap hors-ligne, scan, stock vendeur', () => {
  it('bootstrap : catalogue compact + niveaux du dépôt + favoris', async () => {
    const res = await ctx.agent.get('/api/pos/bootstrap').set(auth(ids.vendorToken));
    expect(res.status).toBe(200);
    expect(res.body.depotId).toBe(ids.depotId);
    expect(res.body.serverTime).toBeTruthy();

    const product = res.body.products.find((p: { name: string }) => p.name === 'Eau Test 1.5L');
    expect(product).toBeTruthy();
    expect(Number(product.selling_price)).toBe(400);
    expect(Array.isArray(product.variants)).toBe(true);

    const level = res.body.levels.find((l: { product_id: string }) => l.product_id === ids.productId);
    expect(Number(level.quantity)).toBe(83);

    expect(res.body.units.length).toBeGreaterThanOrEqual(2);
    expect(res.body.categories.some((c: { name: string }) => c.name === 'Boissons')).toBe(true);
  ...
  it('scan code-barres : produit trouvé, variante trouvée, inconnu 404', async () => {
    const prod = await ctx.agent.get('/api/products/barcode/6100000000011').set(auth(ids.vendorToken));
    ...
    const variantProduct = await ctx.agent
      ...
      .send({ name: 'Parfum Test', sellingPrice: 5000, hasVariants: true, variants: [{ name: '50 ml', barcode: '6001234500017' }] });
    expect(variantProduct.status).toBe(201);
    const variant = await ctx.agent.get('/api/products/barcode/6001234500017').set(auth(ids.vendorToken));
    expect(variant.status).toBe(200);
    expect(variant.body.matched).toBe('variant');
    expect(variant.body.variant_name).toBe('50 ml');

    const unknown = await ctx.agent.get('/api/products/barcode/0000000000000').set(auth(ids.vendorToken));
    ...
    const boot = await ctx.agent.get('/api/pos/bootstrap').set(auth(ids.vendorToken));
    const p = boot.body.products.find((x: { name: string }) => x.name === 'Parfum Test');
    expect(p.variants).toHaveLength(1);
    expect(p.variants[0].barcode).toBe('6001234500017');
  });

  it('stock du dépôt : recherche en ligne simple pour le vendeur', async () => {
    const res = await ctx.agent.get(`/api/depots/${ids.depotId}/stock?search=Eau`).set(auth(ids.vendorToken));
    expect(res.status).toBe(200);
