...
describe("RBAC (corrige SEC-05 : les rôles sont réellement appliqués)", () => {
  ...
  it("un VENDEUR ne peut pas modifier le catalogue", async () => {
    ...
    const archive = await ctx.agent
      .post(`/api/products/${ids.productId}/archive`)
      .set(auth(ids.vendorToken));
  ...
  it("un VENDEUR ne peut pas gérer unités, équipe, dépôts, fournisseurs", async () => {
    const cases: Array<[string, string, object?]> = [
      ...
      [
        "post",
        "/api/users",
        { name: "X", email: "x@x.cm", role: "VENDEUR", depotId: ids.depotId },
      ],
      ...
      [
        "post",
        "/api/stock/adjust",
        { productId: ids.productId, delta: -1, reason: "vol" },
      ],
      [
        "post",
        "/api/stock/receipts",
    ...
    for (const [method, path, body] of cases) {
      const res = await (
        ctx.agent as unknown as Record<
          string,
          (p: string) => ReturnType<typeof ctx.agent.get>
        >
      )[method]!(path)
        .set(auth(ids.vendorToken))
        .send(body ?? {});
  ...
  it("un VENDEUR lit les référentiels et consulte le stock de SON dépôt", async () => {
    expect(
      (await ctx.agent.get("/api/units").set(auth(ids.vendorToken))).status,
    ).toBe(200);
    expect(
      (await ctx.agent.get("/api/depots").set(auth(ids.vendorToken))).status,
    ).toBe(200);
    expect(
      (
        await ctx.agent
    ...
    expect(
      (await ctx.agent.get("/api/categories").set(auth(ids.vendorToken)))
        .status,
    ).toBe(200);
  });
  ...
  it("un ADMIN du tenant ne peut pas toucher la console Super Admin", async () => {
    expect(
      (await ctx.agent.get("/api/tenants").set(auth(ids.adminToken))).status,
    ).toBe(403);
    expect(
      (
        await ctx.agent
    ...
    expect(
      (await ctx.agent.get("/api/configs").set(auth(ids.adminToken))).status,
    ).toBe(403);
    expect(
      (await ctx.agent.get("/api/licenses").set(auth(ids.adminToken))).status,
    ).toBe(403);
  });
});
