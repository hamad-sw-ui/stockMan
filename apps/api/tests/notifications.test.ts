...
describe("Notifications — canaux, dédup exactly-once, supervision (BCK-01/07)", () => {
  it("paramètres : GET crée les défauts, PUT les met à jour (upsert)", async () => {
    ...
    const put = await ctx.agent
      ...
      .set(auth(ids.adminToken))
      .send({
        alertPhone: "+237699000000",
        alertWhatsapp: "+237655000000",
  ...
  it("alerte stock bas : IN_APP + SMS + WhatsApp, jamais en double (clé de dédup)", async () => {
    ...
    const channels = list.body.data
      .map((n: { channel: string }) => n.channel)
      .sort();
    expect(channels).toEqual(["IN_APP", "SMS", "WHATSAPP"]);
    expect(
      list.body.data.every((n: { status: string }) => n.status === "SENT"),
    ).toBe(true);
  ...
  it("centre : marquage lu unitaire puis global", async () => {
    const list = await ctx.agent
      ...
      .set(auth(ids.adminToken));
    const inApp = list.body.data.find(
      (n: { channel: string }) => n.channel === "IN_APP",
    );
    const one = await ctx.agent
    ...
    if (alien.rows[0]) {
      const denied = await ctx.agent
        .patch(`/api/notifications/${alien.rows[0].id}/read`)
        .set(auth(ids.adminToken));
  ...
  it("supervision : interdite aux non-admin éditeur", async () => {
    const denied = await ctx.agent
      .get("/api/notifications/supervision")
      .set(auth(ids.adminToken));
    expect(denied.status).toBe(403);

    const bcrypt = (await import("bcryptjs")).default;
    const saTenant = await ctx.pool.query(
      "INSERT INTO tenants (name) VALUES ('Plateforme') RETURNING id",
    );
    ...
    const saLogin = await ctx.agent
      .post("/api/auth/login")
      .send({ email: "sa-notif@test.cm", password: "Sa123456!" });
