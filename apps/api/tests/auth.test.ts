...
describe("Authentification & sessions", () => {
  it("inscrire un tenant crée admin + licence TRIAL + dépôt + unités", async () => {
    ...
    const depots = await ctx.agent
      .get("/api/depots")
      .set("Authorization", `Bearer ${res.body.accessToken}`);
    expect(depots.body).toHaveLength(1);
    const units = await ctx.agent
      .get("/api/units")
      .set("Authorization", `Bearer ${res.body.accessToken}`);
  ...
  it("login OK / mauvais mot de passe 401", async () => {
    ...
    const ko = await ctx.agent
      .post("/api/auth/login")
      .send({ email, password: "Wrong999!" });
  ...
  it("change-password impose le mot de passe actuel et rouvre une session", async () => {
    ...
    const relog = await ctx.agent
      .post("/api/auth/login")
      .send({ email, password: "Nouveau123!" });
    await ctx.agent
      .post("/api/auth/change-password")
  ...
  it("connexion PIN fonctionne et est validée", async () => {
    ...
    const v = await ctx.agent
      ...
      .send({
        ...
        email: "pin@test.cm",
        role: "VENDEUR",
        depotId: depots.body[0].id,
        pin: "5544",
      });
    expect(v.status).toBe(201);

    const pinLogin = await ctx.agent
      .post("/api/auth/pin")
      .send({ email: "pin@test.cm", pin: "5544" });
    expect(pinLogin.status).toBe(200);
    const wrong = await ctx.agent
      .post("/api/auth/pin")
      .send({ email: "pin@test.cm", pin: "0000" });
    expect(wrong.status).toBe(401);
    const malformed = await ctx.agent
      .post("/api/auth/pin")
      .send({ email: "pin@test.cm", pin: "abc" });
  ...
  it("forgot/reset password ne révèle pas l’existence des comptes", async () => {
    const unknown = await ctx.agent
      ...
      .send({ email: "nobody@test.cm" });
    expect(unknown.status).toBe(200);
    expect(unknown.body.devToken).toBeUndefined();
    const known = await ctx.agent
      .post("/api/auth/forgot-password")
      .send({ email });
    ...
    const relog = await ctx.agent
      .post("/api/auth/login")
      .send({ email, password: "Reset12345!" });
