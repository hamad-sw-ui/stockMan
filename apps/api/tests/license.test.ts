...
describe('Licences (DAT-06 : le middleware de licence est effectif)', () => {
  it('licence TRIAL créée à l’inscription ; les quotas sont contrôlés', async () => {
    ...
    await ctx.pool.query("UPDATE licenses SET max_users=2, max_depots=1, plan_code='TRIAL', status='TRIAL' WHERE tenant_id=$1", [ids.tenantId]);
    const third = await ctx.agent
      .post('/api/users')
      .set(auth(ids.adminToken))
      .send({ name: 'Troisième', email: 'trois@test.cm', role: 'VENDEUR', depotId: ids.depotId });
    expect(third.status).toBe(402);
    expect(third.body.error.code).toBe('LICENSE_LIMIT_USERS');

    // max_depots : 1 dépôt existant → création d’un 2ᵉ bloquée
    const depot = await ctx.agent.post('/api/depots').set(auth(ids.adminToken)).send({ name: 'Dépôt Bureau' });
  ...
  it('licence expirée : écriture bloquée (402), lecture préservée', async () => {
    ...
    const sale = await ctx.agent
      ...
      .send({ items: [{ productId: ids.productId, quantity: 1 }], paymentMethod: 'CASH' });
  ...
  it('renouvellement par le Super Admin débloque l’écriture', async () => {
    ...
    const saLogin = await ctx.agent.post('/api/auth/login').send({ email: 'sa@test.cm', password: 'Sa123456!' });
    ...
    const lic = licenses.body.find((l: { tenant_name: string }) => l.tenant_name === 'SARL Test');
    expect(lic).toBeTruthy();

    const renew = await ctx.agent.post(`/api/licenses/${lic.id}/renew`).set(auth(saToken)).send({ months: 1 });
