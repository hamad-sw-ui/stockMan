...
import { applyMigrations, listMigrationFiles, migrationStatus } from '../src/db/migrations';
...
describe('Chaîne de migrations V2 (rejouable, appliquée sur schéma réel)', () => {
  ...
  it('les tables métier attendues existent', async () => {
    ...
    for (const t of [
      'tenants', 'users', 'refresh_tokens', 'licenses', 'plans', 'depots',
      'categories', 'units', 'products', 'product_variants', 'stock_levels',
      'stock_batches', 'suppliers', 'sales', 'sale_items', 'sale_returns',
      'stock_receipts', 'stock_transfers', 'stock_movements',
      'audit_logs', 'notifications', 'notification_settings', 'system_configs', 'tenant_configs',
    ]) {
