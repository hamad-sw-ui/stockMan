...
async function main() {
  ...
  const password =
    process.env.SA_PASSWORD || crypto.randomBytes(9).toString("base64url");
  if (
    password.length < 8 ||
    ...
    console.error(
      "✖ SA_PASSWORD doit faire 8+ caractères avec au moins une lettre et un chiffre.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (existing.rows[0]) {
      if (existing.rows[0].role !== "SUPER_ADMIN") {
        console.error(
          `✖ ${email} existe déjà avec un rôle non SUPER_ADMIN. Refus par sécurité.`,
        );
        process.exit(1);
      }
      await pool.query(
