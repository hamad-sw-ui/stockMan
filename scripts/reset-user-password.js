...
async function main() {
  ...
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
    console.error('Usage : node scripts/reset-user-password.js utilisateur@exemple.cm');
    process.exit(1);
  }
  ...
  const confirmation = await new Promise((resolve) =>
    rl.question(`⚠️  Pour confirmer, tapez exactement : RESET ${email}\n> `, resolve),
  );
  rl.close();
  ...
  try {
    const user = await pool.query('SELECT id, name, role FROM users WHERE lower(email) = $1', [email]);
    if (!user.rows[0]) {
      console.error(`✖ Aucun utilisateur ${email}.`);
      process.exit(1);
    }
    const temp = crypto.randomBytes(6).toString('base64url') + '1A';
    const hash = await bcrypt.hash(temp, Number(process.env.BCRYPT_ROUNDS) || 10);
    await pool.query('UPDATE users SET password_hash=$2, updated_at=now() WHERE id=$1', [user.rows[0].id, hash]);
    // Révocation de toutes les sessions actives (correction SEC : vol de session)
    await pool.query('UPDATE refresh_tokens SET revoked_at=now() WHERE user_id=$1 AND revoked_at IS NULL', [user.rows[0].id]);
    console.log(`✔ Mot de passe réinitialisé pour ${user.rows[0].name} (${email}) — sessions révoquées.`);
